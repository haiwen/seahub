...
function buildFileViewURL(pathname) {
  return `${siteRoot}lib/${repoID}/file${Utils.encodePath(pathname)}`;
}
...
class FileContent extends React.Component {
  ...
  navigateToSavedAsFile = (nextFileName) => {
    const nextFilePath = Utils.joinPath(window.app.pageOptions.parentDir, nextFileName);
    const nextViewURL = buildFileViewURL(nextFilePath);
    window.location.replace(nextViewURL);
  };
  ...
  onMessage = (event) => {
    ...
    if (message.MessageId === 'Action_Save_Resp') {
      const { success, fileName: savedFileName, FileName, Name } = message.Values || {};
      if (!success) return;

      const nextFileName = savedFileName || FileName || Name;
