...
class ReactImageLightbox extends Component {
  ...
  constructor(props) {
    ...
    this.ocrRecognition = this.ocrRecognition.bind(this);
  ...
  ocrRecognition() {
    ...
    const that = this;
    const { ocrResultCache } = that.state;
    const key = mainSrc + 'i';
    if (ocrResultCache[key]) return;
    this.props.ocrRecognition(mainSrc, (result) => {
      const { ocrResultCache } = that.state;
      const newOcrResultCache = { ...ocrResultCache, [key]: result };
      console.log(newOcrResultCache);
      console.log(result);
      that.setState({ ocrResultCache: newOcrResultCache });
    });
  }

  renderOcrResult(zoomMultiplier) {
    const { mainSrc, animationDisabled, animationDuration } = this.props;
    const { ocrResultCache, rotateDeg, zoomLevel, offsetX, offsetY } = this.state;
    const key = mainSrc + 'i';
    const ocrResult = ocrResultCache[key];
    ...
    return (
      <OCRCanvas data={ocrResult} className="ril-image-current ril__image ocr-result" style={style} />
    );
  }
    ...
    return (
      ...
      >
        ...
        >
          ...
          >
            {images}
            {this.renderOcrResult(zoomMultiplier)}
          </div>
          ...
          >
            ...
            <ul className="ril-toolbar-right ril__toolbarSide ril__toolbarRightSide">
              ...
              {this.props.ocrRecognition && !this.isMobile &&
                <li className="ril-toolbar__item ril__toolbarItem">
                  <button
                    type="button"
                    className="ril-toolbar__item__child ril__toolbarItemChild ril__builtinButton ril__ocrButton"
                    onClick={!isAnimating ? this.ocrRecognition : undefined}
                  />
                </li>
              }
