...
class SidePanel extends Component {
  ...
  constructor(props) {
    super(props);
    const { sidePanelInitData, docUuid } = props;
    this.state = {
  ...
  loadMore = () => {
    if (!this.state.isReloadingData) {
      ...
      this.setState({ isReloadingData: true }, () => {
        seafileAPI.listSdocHistory(this.docUuid, this.currentPage, PER_PAGE).then(res => {
  ...
  renameHistoryVersion = (objID, newName) => {
    seafileAPI.renameSdocHistory(this.docUuid, objID, newName).then((res) => {
      this.setState({
  ...
  showDailyHistory = (path, callback) => {
    ...
    seafileAPI.listSdocDailyHistoryDetail(this.docUuid, dayHistoryGroup.children[0].ctime).then(res => {
