...
import { gettext, siteRoot, sideNavFooterCustomHtml, externalAppLinks } from '../utils/constants';
...
class SideNavFooter extends React.Component {
  ...
  renderExternalAppLinks = () => {
    if (externalAppLinks && (typeof externalAppLinks) === 'object') {
      let keys = Object.keys(externalAppLinks);
      return keys.map((key, index) => {
        return <a key={index} className="item" href={externalAppLinks[key]} aria-hidden="true">{key}</a>;
      });
    }
    return null;
  }
    ...
    return (
      <Fragment>
        <div className="side-nav-footer flex-wrap">
          ...
          <a href={siteRoot + 'download_client_program/'} className={`item ${externalAppLinks ? '' : 'last-item'}`}>
