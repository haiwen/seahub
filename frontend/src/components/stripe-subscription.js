...
import { stripeSubscriptionAPI } from '../utils/stripe-subscription-api';
...
class Plans extends Component {
  ...
  renderPaidOrExtendTime = () => {
    ...
    return (
      <div className='d-flex flex-column subscription-container'>
        <span className="subscription-subtitle">{'Choose a plan'}</span>
        ...
        <span className="subscription-subtitle">{'Preivew'}</span>
        <dl className='items-dl'>
          <div>
            <dd className='order-item order-item-top'>
              <span className='order-into'>{'Plan'}</span>
              <span className='order-value'>{currentPlan.name}</span>
            </dd>
            {isOrgContext &&
              <dd className='order-item'>
                <span className='order-into'>{'Users'}</span>
                <span className='order-value'>{currentPlan.count}</span>
              </dd>
            }
            {/*
            <dd className='order-item'>
              <span className='order-into'>{'Quota'}</span>
        ...
        <Button className='subscription-submit' color="primary" onClick={this.onPay}>{'Submit'}</Button>
      </div>
    );
  };
  ...
  renderAddUser = () => {
    ...
    let operationIntro = 'Add new users';
...
class PlansDialog extends Component {
  ...
  getPlans = () => {
    stripeSubscriptionAPI.getSubscriptionPlans(this.props.paymentType).then((res) => {
      this.setState({
  ...
  onPay = (planID, count, asset_quota, totalAmount) => {
    ...
    let payUrl = serviceURL + '/stripe-subscription/pay/?payment_source=' + this.state.paymentSourceList[0] +
...
class Subscription extends Component {
  ...
  getSubscription = () => {
    stripeSubscriptionAPI.getSubscription().then((res) => {
      ...
      <Fragment>
        <div className="content position-relative" onScroll={this.props.handleContentScroll}>
          <div id="current-plan" className="subscription-info">
            <h3 className="subscription-info-heading">{'Current Plan'}</h3>
            <p className="mb-2">{planName}</p>
          </div>
          {this.props.isOrgContext &&
            <div id="user-limit" className="subscription-info">
              <h3 className="subscription-info-heading">{'User Limit'}</h3>
              <p className="mb-2">{userLimit}</p>
            </div>
          }
          <div id="asset-quota" className="subscription-info">
            <h3 className="subscription-info-heading">{'Quota'}</h3>
            <p className="mb-2">{assetQuota ? assetQuota + 'GB' : '1GB'}</p>
          </div>
          <div id="current-subscription-period" className="subscription-info">
            <h3 className="subscription-info-heading">{'Expiration'}</h3>
            <p className="mb-2">{termEnd}</p>
          </div>
          <div id="product-price" className="subscription-info">
            <h3 className="subscription-info-heading">{'Cloud Service Price Plan'}</h3>
            <p className="mb-2">
              <a rel="noopener noreferrer" target="_blank" href="https://www.seafile.com/en/product/private_server/">{'Get Detail'}</a>
            </p>
          </div>
