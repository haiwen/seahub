...
import { FLOW_NODE_TYPE } from './constants/node';
import { FLOW_EDGE_TYPE } from './constants/edge';
import WorkflowSettingsPanel from './settings-panel';
import { useWorkflows } from './hooks/workflows';
import { EVENT_BUS_TYPE } from './constants/event-bus-type';
import { SETTINGS_MODE } from './constants/panel';
...
const nodeTypes = {
  [FLOW_NODE_TYPE.ADD_NODE_PLACEHOLDER]: AddNodePlaceholder,
  [FLOW_NODE_TYPE.CUSTOM]: CustomNode,
};

const edgeTypes = {
  [FLOW_EDGE_TYPE.CUSTOM]: CustomEdge,
};
...
const WorkflowCanvas = () => {
  const { workflowEventBus, settingsMode } = useWorkflows();
  const [nodes, setNodes] = useNodesState(initialNodes);
  const [edges, setEdges] = useEdgesState(initialEdges);
  const { zoom } = useViewport();

  const settingsPanelTitle = useMemo(() => {
    if (!settingsMode) return '';
    if (settingsMode === SETTINGS_MODE.ADD_NODE) {
      return gettext('Nodes');
    }
    if (settingsMode === SETTINGS_MODE.NODE_SETTINGS) {
      return gettext('Node settings');
    }
    return '';
  }, [settingsMode]);
  ...
  const hideSettingsPanel = useCallback(() => {
    workflowEventBus.dispatch(EVENT_BUS_TYPE.SET_SETTINGS_MODE, null);
  }, [workflowEventBus]);
  ...
  return (
    <div className="workflow-canvas">
      ...
      <WorkflowSettingsPanel show={!!settingsMode} title={settingsPanelTitle} hidePanel={hideSettingsPanel} />
    </div>
  );
};
