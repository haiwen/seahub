...
from seahub.views.file import can_preview_file, can_edit_file, should_use_origin_file_history
        ...
        if operation == 'convert':
            ...
            if dst_type != 'docx':
                ...
                if resp.status_code == 400:
                    error_msg = json.loads(resp.text).get('error_msg', 'File is invalid')
                    return api_error(status.HTTP_400_BAD_REQUEST, error_msg)
...
class FileLockBatchView(APIView):
    """Lock or unlock multiple files in one library.
