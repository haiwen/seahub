...
class OCMSharesView(APIView):
    ...
    def post(self, request):
        ...
        if protocol['name'] == OCM_SEAFILE_PROTOCOL:
            ...
            if 'apiToken' not in protocol['options'].keys():
                error_msg = 'protocol.options.token invalid.'
                return api_error(status.HTTP_400_BAD_REQUEST, error_msg)
...
class OCMSharesPrepareView(APIView):
    ...
    def post(self, request):
        ...
        api_token = OCMShare.objects.gen_token(to_user=to_user, to_server_url=to_server_url)
