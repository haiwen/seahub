...
class RepoFileTagsView(APIView):
    ...
    def get(self, request, repo_id):
        ...
        file_tags = []
        try:
            file_tag_list = FileTags.objects.get_all_tag_by_path(repo_id, file_path)
        except Exception as e:
            logger.error(e)
    ...
    def post(self, request, repo_id):
        ...
        file_tag = FileTags.objects.get_one_file_tag(repo_id, repo_tag_id, file_path)
        if file_tag:
            return Response(status.HTTP_200_OK, 'File_tag Already Exist.')
        ...
        try:
            file_tag = FileTags.objects.add_a_file_tag(repo_id, repo_tag_id, file_path)
        except Exception as e:
            logger.error(e)
            error_msg = 'Internal Server Error.'
            return api_error(status.HTTP_500_INTERNAL_SERVER_ERROR, error_msg)
...
class RepoFileTagView(APIView):
    ...
    def delete(self, request, repo_id, file_tag_id):
        ...
        file_tag = FileTags.objects.get_tag_by_id(file_tag_id)
        if not file_tag:
        ...
        try:
            FileTags.objects.delete_a_file_tag(file_tag_id)
        except Exception as e:
            logger.error(e)
            error_msg = 'Internal Server Error.'
            return api_error(status.HTTP_500_INTERNAL_SERVER_ERROR, error_msg)
