...
class RepoAPITokenAuthentication(BaseAuthentication):
    ...
    Clients should authenticate by passing the token key in the "Authorization"
    HTTP header, prepended with the string "RepoAPIToken ".  For example:

        Authorization: RepoAPIToken 401f7ac837da42b97f613d789819ff93537bee6a
    ...
    * user -- The user to which the token belongs
    ...
    def authenticate(self, request):
        ...
        return key from headers' repo_api_token,
        and set request.boss_behind to person whom repo_api_token was generated by
        ...
        try:
            boss_behind = User.objects.get(email=rat.generated_by)
        except User.DoesNotExist:
            raise AuthenticationFailed('User inactive or deleted')

        request.boss_behind = boss_behind
