...
class FileLabelsManager(models.Manager):
    ...
    def get_all_tag_by_path(self, repo_id, file_path):
        ...
        return super(FileLabelsManager, self).filter(parent_folder_uuid=parent_folder_uuid, file_uuid=file_uuid)

    def get_tag_by_id(self, file_tag_id):
        try:
            return super(FileLabelsManager, self).get(pk=file_tag_id)
        except self.model.DoesNotExist:
            return None

    def add_a_file_tag(self, repo_id, tag_id, file_path):
        file_path = normalize_file_path(file_path)
        filename = os.path.basename(file_path)
        parent_path = os.path.dirname(file_path)
        folder_name = os.path.dirname(parent_path)
        ...
        file_tag = self.model(tag_id=tag_id, file_uuid=file_uuid,
    ...
    def delete_a_file_tag(self, file_tag_id):
        try:
            file_tag = super(FileLabelsManager, self).get(pk=file_tag_id)
            file_tag.delete()
            return True
        except self.model.DoesNotExist:
            return False
...
class FileLabels(models.Model):
    ...
    objects = FileLabelsManager()

    def to_dict(self):
        repo_tag = RepoTags.objects.get_repo_tag_by_id(self.tag_id)
