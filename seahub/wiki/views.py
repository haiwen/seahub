...
def format_markdown_file_content(wiki_id, repo_id, file_path, token, file_response):
    ...
    for img in img_labels:
        ...
        if img_url_re.match(img_url) is not None:
            ...
            new_img_url = '%s/view-image-via-public-wiki/?wiki_id=%s&path=%s' % (service_url, wiki_id, img_path)
    ...
    for a in a_labels:
        ...
        if file_url_re.match(old_href) is not None:
            ...
            if md_url_re.match(old_href) is not None:
                new_href = '%s/published/%s/%s' % (service_url, wiki_id, path)
    ...
    for p in html_content.split('\n'):
        if p.startswith('<h1>') and p.endswith('</h1>'):
            h1_head_content = p.replace('<h1>', '', 1).replace('</h1>', '', 1)
            head = p.replace('<h1>', '<h1 id="user-content-%s">' % p.replace('<h1>', '', 1).replace('</h1>', '', 1), 1)
            html_content = html_content.replace(p, head)
        elif p.startswith('<h2>') and p.endswith('</h2>'):
            head = p.replace('<h2>', '<h2 id="user-content-%s">' % p.replace('<h2>', '', 1).replace('</h2>', '', 1), 1)
            html_content = html_content.replace(p, head)
            outline = '<div class="outline-h2">' + p.replace('<h2>', '', 1).replace('</h2>', '', 1) + '</div>'
            outlines.append(mark_safe(outline))
        elif p.startswith('<h3>') and p.endswith('</h3>'):
            head = p.replace('<h3>', '<h3 id="user-content-%s">' % p.replace('<h3>', '', 1).replace('</h3>', '', 1), 1)
            html_content = html_content.replace(p, head)
            outline = '<div class="outline-h3">' + p.replace('<h3>', '', 1).replace('</h3>', '', 1) + '</div>'
            outlines.append(mark_safe(outline))
...
def slug(request, wiki_id, file_path="home.md"):
    """Show wiki page.
    """
    if not file_path:
        file_path = "home.md"
    ...
    if is_dir is False and file_type == MARKDOWN:
        ...
        if file_response:
            file_content, h1_head_content, outlines, err_msg = format_markdown_file_content(
                wiki_id, wiki.repo_id, file_path, fs.token, file_response)
...
def edit_slug(request, wiki_id, file_path="home.md"):
    ...
    #     if len(file_path.split('.')) == 1:
    #         new_path = file_path[1:] + '.md'
    #         url = reverse('edit_slug', args=[wiki_id, new_path])
