...
CACHE_DIR = "/tmp"
install_topdir = os.path.expanduser(
...
FREEZE_USER_ON_LOGIN_FAILED = (
    False  # deactivate user account when login attempts exceed limit
)
...
SEND_EMAIL_ON_ADDING_SYSTEM_MEMBER = (
    True  # Whether to send email when a system staff adding new member.
)
SEND_EMAIL_ON_RESETTING_USER_PASSWD = (
    True  # Whether to send email when a system staff resetting user's password.
)
...
EVENTS_CONFIG_FILE = os.environ.get(
    "EVENTS_CONFIG_FILE",
    os.path.join(d(d(d(d(os.path.abspath(__file__))))), "conf", "seafevents.conf"),
)
...
# baidu netdisk oauth settings
BAIDU_NETDISK_APP_KEY = "Gk1rBoQFFT79Os92W4dxNGukUx5MtQ50"
BAIDU_NETDISK_APP_SECRET = "hHUxop1HoNQZVK8W5ast6RZ6Q2X3ZbRF"
BAIDU_NETDISK_REDIRECT_URI = "oob"
# 百度网盘代理模块URL
NDPROXY_URL = ""
# 是否验证NDPROXY模块的SSL证书（自签名证书设为False）
NDPROXY_SSL_VERIFY = False
...
if (
    ENABLE_OAUTH
    or ENABLE_CUSTOM_OAUTH
    or ENABLE_WORK_WEIXIN
    or ENABLE_WEIXIN
    or ENABLE_DINGTALK
):
