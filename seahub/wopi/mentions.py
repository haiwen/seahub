...
def cache_wopi_mentions(access_token, request_user, repo_id, file_path, mentioned_users, sequence, org_id=None):
    cache_key = generate_wopi_mentions_cache_key(access_token)
    ...
    try:
        ...
        if sequence <= state['last_flushed_sequence'] or sequence < state['sequence']:
            return state
...
def flush_cached_wopi_mentions(access_token, request_user, repo_id, file_path, org_id=None):
    ...
    try:
        ...
        state.update({
            ...
            'last_flushed_sequence': state['sequence'],
            'updated_at': int(time.time()),
        })
        cache.set(cache_key, state, WOPI_MENTION_CACHE_EXPIRATION)
