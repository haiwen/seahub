...
from seahub.thumbnail.apis import ThumbnailGet

from .views import thumbnail_create, share_link_thumbnail_get, \
    share_link_thumbnail_create

urlpatterns = [
    re_path(r'^(?P<repo_id>[-0-9a-f]{36})/create/$', thumbnail_create, name='thumbnail_create'),
    re_path(r'^(?P<repo_id>[-0-9a-f]{36})/(?P<size>[0-9]+)/(?P<path>.*)$', ThumbnailGet.as_view(), name='thumbnail_get'),
