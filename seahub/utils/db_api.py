...
class SeafileDB:
    ...
    def get_users_by_quota_alert(self, threshold=0.9):
        
        users_quota_list_sql = f"""
            SELECT
              uq.`user` as email,
              SUM(rs.`size`) as quota_usage,
              uq.`quota` as quota_total,
              SUM(rs.`size`) / uq.`quota` AS usage_ratio
            FROM
              `{self.db_name}`.`RepoSize` rs
            JOIN
              `{self.db_name}`.`RepoOwner` ro ON rs.`repo_id` = ro.`repo_id`
            JOIN
              `{self.db_name}`.`UserQuota` uq ON ro.`owner_id` = uq.`user`
            LEFT JOIN
              `{self.db_name}`.`VirtualRepo` v ON v.repo_id = ro.repo_id
        ...
        org_users_quota_list_sql = f"""
            ...
            FROM
              `{self.db_name}`.`RepoSize` rs
            JOIN
              `{self.db_name}`.`OrgRepo` ro ON rs.`repo_id` = ro.`repo_id`
            JOIN
              `{self.db_name}`.`OrgUserQuota` uq ON ro.`user` = uq.`user`
            LEFT JOIN
		          `{self.db_name}`.`VirtualRepo` v ON v.repo_id = ro.repo_id
            WHERE v.repo_id is NULL
            GROUP BY
        ...
        with connection.cursor() as cursor:
            users = []
            cursor.execute(users_quota_list_sql)
            for item in cursor.fetchall():
                email = item[0]
                quota_usage = item[1]
                quota_total = item[2]
            ...
            cursor.execute(org_users_quota_list_sql)
            for item in cursor.fetchall():
                email = item[0]
                quota_usage = item[1]
                quota_total = item[2]
