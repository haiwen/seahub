...
def revert_nav(navigation, page_id, flag=0):
    """
    revert nav
    If the parent or one of the ancestors is deleted, return 1，
    and reset the is_delete to false of a page = page_id
    """
    if flag != 1:
        for nav in navigation:
...
def remove_flags(data):
    ...
    # remove the useless "flag" key generated by function revert_nav
    for item in data:
        if 'flag' in item:
            del item['flag']
        if 'children' in item:
