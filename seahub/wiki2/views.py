...
from seahub.wiki2.models import Wiki2 as Wiki, Wiki2Publish, WikiFileViews, Wiki2Settings
...
# SSR proxy (imported lazily so the site still works when wiki-ssr is absent)
try:
    from seahub.wiki2.ssr_proxy import proxy_to_ssr
    _HAS_SSR_PROXY = True
except ImportError:
    _HAS_SSR_PROXY = False
...
def wiki_publish_view(request, publish_url, page_id=None):
    ...
    SSR strategy
    ────────────
    When WIKI_SSR_SERVER_URL is configured the response is generated by the
    Next.js SSR service so that search-engine crawlers receive fully-rendered
    HTML.  The SSR service calls back into Seahub's own APIs over the internal
    network.

    The original Django template render is kept as a fallback: if the SSR
    ...
    if _HAS_SSR_PROXY:
        ...
        logger.info(
            '[wiki_publish_view] SSR proxy failed for publish_url=%s page_id=%s; '
            'falling back to template render.',
            publish_url, page_id,
        )
