    ...
    var DirentGridView = Backbone.View.extend({
        ...
        multiChoose: function(e) {
            ...
            if (e.which == 1 && dirView.$multi_choose == true) {
                this.$('.grid-img-container').toggleClass('hl');
                this.$('.grid-img-inner-container').removeClass('hl');
        ...
        rename: function() {
            ...
            var $grid_hidden_op = this.$('.grid-hidden-op'),
                $name = this.$('.grid-link'),
                $img_container = this.$('.grid-img-container'),
                $name_container = this.$('.grid-link-container');

            $name_container.css('width', $('.grid-item').width()).css('margin', 0);
            $grid_hidden_op.addClass('hide');
            $img_container.addClass('hl');
            $name_container.prepend(form);
            $name.hide();

            var cancelRename = function() {
                $name.css('width', '104').show();
                form.remove();
                $img_container.removeClass('hl');
                $op.css({
                    'width': 104,
                    'margin': '0 auto'
                });
