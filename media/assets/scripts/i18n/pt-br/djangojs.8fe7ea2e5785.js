

(function(globals) {

  var django = globals.django || (globals.django = {});

  
  django.pluralidx = function(n) {
    var v=(n > 1);
    if (typeof(v) == 'boolean') {
      return v ? 1 : 0;
    } else {
      return v;
    }
  };
  

  /* gettext library */

  django.catalog = django.catalog || {};
  
  var newcatalog = {
    "%curr% of %total%": "%curr% do %total%", 
    "(at least {passwordLength} characters)": "(\u81f3\u5c11 {passwordLength} \u4e2a\u5b57\u7b26)", 
    "(at least {placeholder} characters)": "\uff08\u81f3\u5c11 {placeholder} \u4e2a\u5b57\u7b26\uff09", 
    "(current version)": "(vers\u00e3o atual)", 
    "1 month ago": "1 mes atr\u00e1s", 
    "1 week ago": "1 semana atr\u00e1s", 
    "3 days ago": "3 dias atr\u00e1s", 
    "<a href=\"%url%\" target=\"_blank\">The image</a> could not be loaded.": "<a href=\"%url%\" target=\"_blank\">A imagem</a> n\u00e3o pode ser carregada.", 
    "A file is being uploaded. Are you sure you want to leave this page?": "\u6b63\u5728\u4e0a\u4f20\u4e00\u4e2a\u6587\u4ef6\u3002\u4f60\u786e\u5b9a\u8981\u79bb\u5f00\u8fd9\u5f53\u524d\u7f51\u9875\u5417?", 
    "A file named {upload_file_link} is uploaded to {uploaded_link}.": "\u65b0\u6587\u4ef6 {upload_file_link} \u4e0a\u4f20\u5230\u4e86 {uploaded_link}\u3002", 
    "A file with the same name already exists in this folder.": "Um arquivo com o mesmo nome j\u00e1 existe nesta pasta", 
    "About": "Sobre", 
    "About Us": "Sobre", 
    "Accepted": "Aceito", 
    "Access Log": "Registro de acesso", 
    "Action": "\u64cd\u4f5c", 
    "Actions": "A\u00e7\u00f5es", 
    "Active": "Ativo", 
    "Active Users": "Usu\u00e1rios ativos", 
    "Activities": "Atividades", 
    "Add Admins": "Incluir Administradores", 
    "Add File": "\u6dfb\u52a0\u6587\u6863", 
    "Add Library": "Adicionar  biblioteca", 
    "Add Member": "Adicionar Membro", 
    "Add User": "Adicionar Usu\u00e1rio", 
    "Add a comment.": "\u6dfb\u52a0\u8bc4\u8bba", 
    "Add admin": "Incluir administrador", 
    "Add auto expiration": "Adicionar auto expira\u00e7\u00e3o", 
    "Add group member": "\u589e\u52a0\u7fa4\u7ec4\u6210\u5458", 
    "Add new reviewer": "\u6dfb\u52a0\u8bc4\u5ba1\u4eba", 
    "Add password protection": "Adicionar prote\u00e7\u00e3o por senha", 
    "Add user": "Incluir usu\u00e1rios", 
    "Added user {user}": "Usu\u00e1rio adicionado {user}", 
    "Admin": "Administrador", 
    "Admin Logs": "Logs do Administrador", 
    "Admin access": "\u53ef\u7ba1\u7406", 
    "All": "Todos", 
    "All Groups": "Todos os grupos", 
    "All Libraries": "\u5168\u90e8\u8d44\u6599\u5e93", 
    "All Public Links": "Todos links p\u00fablicos", 
    "All file types": "Todos os tipos de arquivos", 
    "An integer that is greater than 0 or equal to -2.": "-2 \u6216\u5927\u4e8e 0 \u7684\u6574\u6570\u3002", 
    "An internal link is a link to a file or folder that can be accessed by users with read permission to the file or folder.": "\u5185\u90e8\u94fe\u63a5\u662f\u6307\u5411\u6587\u4ef6\u6216\u76ee\u5f55\u7684\u94fe\u63a5\uff0c\u53ea\u6709\u5bf9\u8be5\u6587\u4ef6\u6216\u76ee\u5f55\u6709\u8bbf\u95ee\u6743\u9650\u7684\u4eba\u53ef\u4ee5\u8bbf\u95ee\u3002", 
    "Anonymous User": "Usu\u00e1rio an\u00f4nimo", 
    "Are you sure to delete": "\u786e\u5b9a\u8981\u5220\u9664", 
    "Are you sure to revoke access of user {placeholder} ?": "\u60a8\u786e\u5b9a\u8981\u64a4\u6d88\u7528\u6237 {placeholder}\u7684\u8bbf\u95ee\u6743\u9650\u5417\uff1f", 
    "Are you sure you want to clear trash?": "Voc\u00ea tem certeza que deseja limpar a lixeira?", 
    "Are you sure you want to delete %s ?": "Voc\u00ea tem certeza que deseja excluir %s ?", 
    "Are you sure you want to delete %s completely?": "Voc\u00ea tem certeza que deseja excluir %s completamente?", 
    "Are you sure you want to delete all %s's libraries?": "Voc\u00ea tem certeza que deseja excluir todas as bibliotecas de %s?", 
    "Are you sure you want to delete the share link?": "\u786e\u5b9a\u5220\u9664\u8be5\u5171\u4eab\u94fe\u63a5\u5417\uff1f", 
    "Are you sure you want to delete these selected items?": "Tem certeza que deseja excluir os items selecionados?", 
    "Are you sure you want to delete {placeholder} ?": "\u786e\u5b9a\u8981\u5220\u9664 {placeholder} \u5417\uff1f", 
    "Are you sure you want to disconnect?": "\u4f60\u786e\u5b9a\u8981\u65ad\u5f00\u8fde\u63a5\u5417\uff1f", 
    "Are you sure you want to quit this group?": "Voc\u00ea tem certeza que deseja sair deste grupo?", 
    "Are you sure you want to remove the share link?": "\u786e\u5b9a\u5220\u9664\u8be5\u5171\u4eab\u94fe\u63a5\u5417\uff1f", 
    "Are you sure you want to restore %s?": "Voc\u00ea tem certeza que deseja restaurar %s?", 
    "Are you sure you want to restore this library?": "Tem certeza que deseja recuperar essa biblioteca?", 
    "Are you sure you want to unlink this device?": "Voc\u00ea tem certeza que deseja desconectar este dispositivo?", 
    "Are you sure you want to unpublish this library?": "\u60a8\u786e\u5b9a\u8981\u53d6\u6d88\u53d1\u5e03\u6b64\u8d44\u6599\u5e93\u5417\uff1f", 
    "Are you sure you want to unshare %s ?": "Voc\u00ea tem certeza que quer descompartilhar %s?", 
    "Audio": "\u00c1udio", 
    "Author": "\u6240\u6709\u8005", 
    "Avatar": "Avatar", 
    "Avatar:": "Avatar:", 
    "Back": "Voltar", 
    "Back to parent directory": "\u8fd4\u56de\u4e0a\u7ea7\u76ee\u5f55", 
    "Besides Write permission, user can also share the library.": "\u9664\u4e86\u53ef\u8bfb\u5199\u6743\u9650\uff0c\u7528\u6237\u8fd8\u53ef\u4ee5\u5171\u4eab\u8d44\u6599\u5e93\u3002", 
    "Broken (please contact your administrator to fix this library)": "Danificado (por favor contate seu administrador para consertar esta biblioteca)", 
    "By name ascending": "\u6309\u540d\u79f0\u5347\u5e8f", 
    "By name descending": "\u6309\u540d\u79f0\u964d\u5e8f", 
    "By size ascending": "\u6309\u5927\u5c0f\u5347\u5e8f", 
    "By size descending": "\u6309\u5927\u5c0f\u964d\u5e8f", 
    "By time ascending": "\u6309\u65f6\u95f4\u5347\u5e8f", 
    "By time descending": "\u6309\u65f6\u95f4\u964d\u5e8f", 
    "Can not copy directory %(src)s to its subdirectory %(des)s": "N\u00e3o \u00e9 poss\u00edvel copiar o diret\u00f3rio %(src)s para o subdiret\u00f3rio %(des)s", 
    "Can not move directory %(src)s to its subdirectory %(des)s": "N\u00e3o \u00e9 poss\u00edvel mover o diret\u00f3rio %(src)s para o subdiret\u00f3rio %(des)s", 
    "Can not select self as a related file.": "\u4e0d\u80fd\u9009\u62e9\u81ea\u5df1\u4f5c\u4e3a\u76f8\u5173\u6587\u6863\u3002", 
    "Cancel": "Cancelar", 
    "Cancel All": "Cancelar Todos", 
    "Canceled.": "Cancelados", 
    "Change Password": "Trocar senha", 
    "Change Password of Library {placeholder}": "Alterar senha da Biblioteca {placeholder}", 
    "Changes": "\u6539\u52a8", 
    "Clean": "Limpo", 
    "Clean succeeded.": "\u6e05\u9664\u6210\u529f\u3002", 
    "Cleaned trash": "\u6e05\u7a7a\u56de\u6536\u7ad9", 
    "Clear Trash": "Limpar Lixeira", 
    "Clear files in trash and history\uff1a": "Limpar lixeira e historico :", 
    "Clients": "Clientes", 
    "Close": "Fechar", 
    "Close (Esc)": "Fechar(Esc)", 
    "Co-Owner": "\u5171\u540c\u6240\u6709\u8005", 
    "Column": "\u5206\u680f", 
    "Comment": "Coment\u00e1rio", 
    "Comments": "Coment\u00e1rios", 
    "Confirm Password": "Confirme a senha", 
    "Connect": "\u8fde\u63a5", 
    "Contact Email": "E-mail de contato", 
    "Contact Email:": "E-mail de contato:", 
    "Content Scan Records": "\u6587\u4ef6\u626b\u63cf\u8bb0\u5f55", 
    "Contents": "\u5185\u5bb9", 
    "Copy": "Copiar", 
    "Copy selected item(s) to:": "Copiar itens selecionados para:", 
    "Copy {num} items": "\u590d\u5236 {num} \u4e2a\u6761\u76ee", 
    "Copy {placeholder} to": "\u590d\u5236 {placeholder} \u5230", 
    "Copy {placeholder} to:": "Copiar {placeholder} para:", 
    "Copying %(name)s": "Copiando %(name)s", 
    "Copying file %(index)s of %(total)s": "Copiando %(index)s Arquivos de %(total)s", 
    "Count": "Contar", 
    "Create At / Last Login": "Criado em / \u00daltimo Login", 
    "Create Group": "Criar Grupo", 
    "Create Library": "\u521b\u5efa\u8d44\u6599\u5e93", 
    "Create a draft from this file, instead of editing it directly.": "\u4ece\u6b64\u6587\u4ef6\u521b\u5efa\u8349\u7a3f\uff0c\u800c\u4e0d\u662f\u76f4\u63a5\u7f16\u8f91\u5b83\u3002", 
    "Create a new tag": "\u65b0\u5efa\u6807\u7b7e", 
    "Created At": "Criado em", 
    "Created Files": "\u521b\u5efa\u7684\u6587\u4ef6", 
    "Created draft": "\u521b\u5efa\u8349\u7a3f", 
    "Created file": "\u521b\u5efa\u6587\u4ef6", 
    "Created folder": "\u521b\u5efa\u6587\u4ef6\u5939", 
    "Created group {group_name}": "Grupo Criado {group_name}", 
    "Created library": "Biblioteca criada", 
    "Created library {library_name} with {owner} as its owner": "\u521b\u5efa\u8d44\u6599\u5e93 {library_name} \u5e76\u628a\u8d44\u6599\u5e93\u62e5\u6709\u8005\u8bbe\u4e3a {owner}", 
    "Created {n} files": "\u521b\u5efa {n} \u4e2a\u6587\u4ef6", 
    "Creator": "Criador", 
    "Current Library": "Biblioteca atual", 
    "Current Path: ": "Caminho atual:", 
    "Current Version": "Vers\u00e3o Atual", 
    "Current path: ": "Caminho atual:", 
    "Custom file types": "Customizar tipos de arquivo", 
    "Date": "Data", 
    "Delete": "Excluir", 
    "Delete Account": "Excluir Conta", 
    "Delete Department": "\u5220\u9664\u90e8\u95e8", 
    "Delete File": "\u5220\u9664\u6587\u4ef6", 
    "Delete Folder": "\u5220\u9664\u6587\u4ef6\u5939", 
    "Delete Group": "Excluir Grupo", 
    "Delete Items": "Excluir items", 
    "Delete Library": "Excluir Biblioteca", 
    "Delete Library By Owner": "Excluir Biblioteca Por Propriet\u00e1rio", 
    "Delete Member": "Excluir Membro", 
    "Delete Time": "Excluir hora", 
    "Delete User": "Excluir Usu\u00e1rio", 
    "Delete draft": "\u5220\u9664\u8349\u7a3f", 
    "Delete failed": "Exclus\u00e3o falhou", 
    "Delete files from this device the next time it comes online.": "Excluir arquivos deste dispositivo da pr\u00f3xima vez em que ele estiver online.", 
    "Deleted Libraries": "\u5df2\u5220\u9664\u7684\u8d44\u6599\u5e93", 
    "Deleted Time": "Excluir hora", 
    "Deleted directories": "Diret\u00f3rios exclu\u00eddos", 
    "Deleted draft": "\u5220\u9664\u8349\u7a3f", 
    "Deleted file": "\u5220\u9664\u6587\u4ef6", 
    "Deleted files": "Arquivos exclu\u00eddo", 
    "Deleted folder": "\u5220\u9664\u6587\u4ef6\u5939", 
    "Deleted group {group_name}": "Grupo exclu\u00eddo {group_name}", 
    "Deleted library": "Biblioteca exclu\u00edda", 
    "Deleted library {library_name}": "Biblioteca exclu\u00edda {library_name}", 
    "Deleted user {user}": "Usu\u00e1rio exclu\u00eddo {user}", 
    "Departments": "\u7ec4\u7ec7\u67b6\u6784", 
    "Description": "Descri\u00e7\u00e3o", 
    "Detail": "Detalhes", 
    "Details": "Detalhes", 
    "Device": "Dispositivo", 
    "Device Name": "Nome do Dispositivo", 
    "Devices": "Dispositivos", 
    "Diff": "Diferen\u00e7a", 
    "Direct Download Link:": "\u76f4\u63a5\u4e0b\u8f7d\u94fe\u63a5\uff1a", 
    "Direct download link is copied to the clipboard.": "\u76f4\u63a5\u4e0b\u8f7d\u8fde\u63a5\u5df2\u590d\u5236\u5230\u526a\u5207\u677f\u3002", 
    "Directory": "Diret\u00f3rio", 
    "Disable Two-Factor Authentication": "Desabilitar Autentica\u00e7\u00e3o de 2 fatores", 
    "Disconnect": "\u65ad\u5f00\u8fde\u63a5", 
    "Dismiss Group": "Ignorar Grupo", 
    "Document": "\u6587\u6863", 
    "Document convertion failed.": "A convers\u00e3o falhou", 
    "Documents": "Documentos", 
    "Don't keep history": "N\u00e3o manter hist\u00f3rico", 
    "Don't replace": "N\u00e3o Substituir", 
    "Don't send emails": "\u4e0d\u53d1\u9001\u90ae\u4ef6", 
    "Download": "Download", 
    "Draft": "\u8349\u7a3f", 
    "Draft has been deleted.": "\u8349\u7a3f\u6587\u4ef6\u5df2\u88ab\u5220\u9664\u3002", 
    "Draft is a way to let you collaborate with others on files. You can create a draft from a file, edit the draft and then ask for a review. The original file will be updated only after the draft has been reviewed.": "\u4f60\u53ef\u4ee5\u4f7f\u7528\u8349\u7a3f\u548c\u4ed6\u4eba\u8fdb\u884c\u6587\u4ef6\u534f\u540c\u7f16\u8f91\u3002\u4f60\u53ef\u4ee5\u521b\u5efa\u8349\u7a3f\u3001\u7f16\u8f91\u8349\u7a3f\uff0c\u7136\u540e\u8981\u6c42\u8bc4\u5ba1\u3002\u539f\u59cb\u6587\u4ef6\u53ea\u6709\u5728\u8349\u7a3f\u5ba1\u6838\u540e\u624d\u4f1a\u66f4\u65b0\u3002", 
    "Drafts": "\u8349\u7a3f", 
    "Edit": "Editar", 
    "Edit Draft": "\u7f16\u8f91\u8349\u7a3f", 
    "Edit File": "\u7f16\u8f91\u6587\u4ef6", 
    "Edit Tag": "\u4fee\u6539\u6807\u7b7e", 
    "Edit failed": "Edi\u00e7\u00e3o falhou", 
    "Edit failed.": "Edi\u00e7\u00e3o falhou.", 
    "Edit labels": "\u7f16\u8f91\u6807\u7b7e", 
    "Edit on cloud and download": "\u4e91\u7aef\u7f16\u8f91\u4e0e\u4e0b\u8f7d", 
    "Edit on cloud only": "\u4ec5\u4e91\u7aef\u7f16\u8f91", 
    "Edit succeeded.": "\u6743\u9650\u4fee\u6539\u6210\u529f\u3002", 
    "Edit-on-Cloud": "\u4ec5\u4e91\u7aef\u8bfb\u5199", 
    "Email": "E-mail", 
    "Email Notification": "\u90ae\u4ef6\u901a\u77e5", 
    "Email Notification of File Changes": "\u6587\u4ef6\u6539\u52a8\u90ae\u4ef6\u901a\u77e5", 
    "Email is invalid.": "\u90ae\u7bb1\u65e0\u6548\u3002", 
    "Emails": "\u90ae\u7bb1", 
    "Emails, separated by ','": "\u8f93\u5165\u90ae\u7bb1\uff0c\u591a\u4e2a\u4ee5,\u5206\u9694", 
    "Empty file upload result": "Resultado do carregamento de arquivos vazio", 
    "Enable Two-Factor Authentication": "Habilitar Autentica\u00e7\u00e3o de 2 fatores", 
    "Encrypt": "Criptografar", 
    "Encrypted library": "Biblioteca encriptada", 
    "Error": "Erro", 
    "Exit Institution Admin": "\u9000\u51fa\u673a\u6784\u7ba1\u7406", 
    "Exit Organization Admin": "\u9000\u51fa\u673a\u6784\u7ba1\u7406", 
    "Exit System Admin": "Sair da Administra\u00e7\u00e3o", 
    "Expiration": "Expira\u00e7\u00e3o", 
    "Expiration Date:": "\u8fc7\u671f\u65f6\u95f4\uff1a", 
    "Expired": "Expirado", 
    "Failed to copy %(name)s": "Falha ao copiar %(name)s", 
    "Failed to copy %(name)s and %(amount)s other item(s).": "\u590d\u5236 %(name)s \u548c\u5176\u4ed6 %(amount)s \u9879\u6761\u76ee\u5931\u8d25\u3002", 
    "Failed to copy %(name)s.": "\u590d\u5236 %(name)s \u9879\u6761\u76ee\u5931\u8d25\u3002", 
    "Failed to copy files to another library.": "\u6587\u4ef6\u590d\u5236\u5931\u8d25\u3002", 
    "Failed to delete %(name)s and %(amount)s other items.": "Falha ao excluir %(name)s e %(amount)s outros items.", 
    "Failed to delete %(name)s and 1 other item.": "Falha ao excluir %(name)s e 1 outro items", 
    "Failed to delete %(name)s.": "Falha ao excluir %(name)s.", 
    "Failed to delete draft %(draft)s.": "\u5220\u9664\u8349\u7a3f %(draft)s\u5931\u8d25\u3002", 
    "Failed to delete {name}": "\u5220\u9664 {name} \u5931\u8d25", 
    "Failed to delete {name} and other {n} items.": "\u5220\u9664 {name} \u4ee5\u53ca\u53e6\u5916 {n} \u9879\u5931\u8d25\u3002", 
    "Failed to delete {name}.": "\u5220\u9664 {name} \u5931\u8d25\u3002", 
    "Failed to move %(name)s": "Falha ao mover %(name)s", 
    "Failed to move %(name)s and %(amount)s other item(s).": "\u79fb\u52a8 %(name)s \u548c\u5176\u4ed6 %(amount)s \u9879\u6761\u76ee\u5931\u8d25\u3002", 
    "Failed to move %(name)s.": "\u79fb\u52a8 %(name)s \u9879\u6761\u76ee\u5931\u8d25\u3002", 
    "Failed to move files to another library.": "\u6587\u4ef6\u79fb\u52a8\u5931\u8d25\u3002", 
    "Failed to move {name}.": "\u79fb\u52a8 {name} \u5931\u8d25\u3002", 
    "Failed to publish draft %(draft)s.": "\u53d1\u5e03\u8349\u7a3f %(draft)s\u5931\u8d25\u3002", 
    "Failed to send to {email_placeholder}: {errorMsg_placeholder}": "\u53d1\u9001\u7ed9  {email_placeholder}\u5931\u8d25\uff1a{errorMsg_placeholder}", 
    "Failed to send to {placeholder}": "Falha ao enviar para  {placeholder}", 
    "Failed to unshare {name}": "{name} \u53d6\u6d88\u5171\u4eab\u5931\u8d25", 
    "Failed.": "Falhou", 
    "Failed. Please check the network.": "Falha. Por favor, verifique a rede.", 
    "Favorites": "Favoritos", 
    "File": "Arquivo", 
    "File Access": "\u6587\u4ef6\u8bbf\u95ee", 
    "File History": "\u6587\u4ef6\u5386\u53f2", 
    "File Scan": "\u6587\u4ef6\u626b\u63cf", 
    "File Types": "\u6587\u4ef6\u7c7b\u578b", 
    "File Update": "\u6587\u4ef6\u66f4\u65b0", 
    "File Upload": "Carregamento de Arquivo", 
    "File Upload canceled": "Upload de Arquivo cancelado", 
    "File Upload complete": "Upload de Arquivo completo", 
    "File Upload failed": "Carregamento de Arquivo falhou", 
    "File Uploading...": "Carregando Arquivo...", 
    "File does not exist.": "\u6587\u4ef6\u4e0d\u5b58\u5728\u3002", 
    "File download is disabled: the share link traffic of owner is used up.": "O download do arquivo foi desativado: o  tr\u00e1fego do  link compartilhado se esgotou.", 
    "File extensions can only be {placeholder}.": " \u6587\u4ef6\u6269\u5c55\u540d\u53ea\u80fd\u662f {placeholder}\u3002", 
    "File is locked": "Arquivo bloqueado", 
    "File is locked by others.": "\u6587\u4ef6\u88ab\u5176\u4ed6\u4eba\u9501\u5b9a\u4e86\u3002", 
    "File is too big": "Arquivo muito grande", 
    "File is too small": "Arquivo muito pequeno", 
    "File {file_link} has a new comment form user {author}.": "\u7528\u6237 {author} \u7ed9\u6587\u4ef6 {file_link} \u6dfb\u52a0\u4e86\u4e00\u4e2a\u65b0\u7684\u8bc4\u8bba\u3002", 
    "Files": "Arquivos", 
    "Filetype not allowed": "Tipo de arquivo n\u00e3o permitido", 
    "Folder": "Pasta", 
    "Folder Permission": "Permiss\u00e3o da Pasta", 
    "Folder Premission": "\u6587\u4ef6\u5939\u6743\u9650", 
    "Folder does not exist.": "\u6587\u4ef6\u5939\u4e0d\u5b58\u5728\u3002", 
    "Folders": "Pastas", 
    "Generate": "Gerar", 
    "Global Address Book": "\u901a\u8baf\u5f55", 
    "Go to Library": "\u524d\u5f80\u8d44\u6599\u5e93", 
    "Greater than or equal to": "\u5927\u4e8e\u6216\u7b49\u4e8e", 
    "Grid": "Grid", 
    "Group": "Grupo", 
    "Group Permission": "Permiss\u00e3o de Grupo", 
    "Group not found": "\u672a\u627e\u5230\u7fa4\u7ec4", 
    "Groups": "Grupos", 
    "Groups allow multiple people to collaborate on libraries. Groups you join will be listed here.": "\u7fa4\u7ec4\u5141\u8bb8\u591a\u4eba\u534f\u4f5c\u529e\u516c\u3002\u4f60\u52a0\u5165\u7684\u7fa4\u7ec4\u4f1a\u5728\u8fd9\u513f\u5217\u51fa\u3002", 
    "Groups allow multiple people to collaborate on libraries. You can create a group by clicking the \"New Group\" button.": "\u7fa4\u7ec4\u5141\u8bb8\u591a\u4eba\u534f\u4f5c\u529e\u516c\u3002\u4f60\u53ef\u4ee5\u901a\u8fc7\u70b9\u51fb\u201c\u65b0\u5efa\u7fa4\u7ec4\u201d\u6309\u626d\u6765\u521b\u5efa\u4e00\u4e2a\u7fa4\u7ec4\u3002", 
    "Help": "Ajuda", 
    "Hide": "Ocultar", 
    "History": "Hist\u00f3rico", 
    "History Setting": "Configura\u00e7\u00f5es do Hist\u00f3rico", 
    "History Versions": "\u5386\u53f2\u7248\u672c", 
    "IP": "IP", 
    "If the share link is deleted, no one will be able to access it any more.": "\u5982\u679c\u5220\u9664\uff0c\u83b7\u53d6\u8be5\u8fde\u63a5\u7684\u4efb\u4f55\u4eba\u5c06\u65e0\u6cd5\u7ee7\u7eed\u8bbf\u95ee\u3002", 
    "If the share link is removed, no one will be able to access it any more.": "\u5982\u679c\u5220\u9664\uff0c\u83b7\u53d6\u8be5\u8fde\u63a5\u7684\u4efb\u4f55\u4eba\u5c06\u65e0\u6cd5\u7ee7\u7eed\u8bbf\u95ee\u3002", 
    "If you don't have any device with you, you can access your account using backup codes.": "Se voc\u00ea n\u00e3o tiver nenhum dispositivo contigo, voc\u00ea pode acessar a sua conta utilizando c\u00f3digos de backup", 
    "Image": "Imagem", 
    "Images": "Imagens", 
    "In all libraries": "Em todas as bibliotecas", 
    "Inactive": "Inativo", 
    "Indexing...": "\u5efa\u7acb\u7d22\u5f15...", 
    "Info": "Informa\u00e7\u00e3o", 
    "Input file extensions here, separate with ','": "Digite as extens\u00f5es de arquivos aqui, separado por ','", 
    "Institution Admin": "\u673a\u6784\u7ba1\u7406", 
    "Institutions": "Institui\u00e7\u00f5es", 
    "Internal Link": "\u5185\u90e8\u94fe\u63a5", 
    "Internal Server Error": "Erro interno do servidor", 
    "Internal error. Failed to copy %(name)s and %(amount)s other item(s).": "Erro intero. Falha ao copiar %(name)s e %(amount)s outros item(s).", 
    "Internal error. Failed to copy %(name)s.": "Erro interno. Falhou ao copiar %(name)s.", 
    "Internal error. Failed to move %(name)s and %(amount)s other item(s).": "Erro interno. Falha ao mover %(name)s e %(amount)s outros item(s).", 
    "Internal error. Failed to move %(name)s.": "Erro interno. Falhou ao mover %(name)s.", 
    "Internal link copied to clipboard": "\u5185\u90e8\u94fe\u63a5\u5df2\u7ecf\u88ab\u590d\u5236\u5230\u526a\u5207\u677f", 
    "Internal link has been copied to clipboard": "\u5185\u90e8\u94fe\u63a5\u5df2\u590d\u5236\u5230\u526a\u8d34\u677f", 
    "Invalid destination path": "Caminho de destino inv\u00e1lido", 
    "Invalid file size range.": "\u6587\u4ef6\u5927\u5c0f\u8303\u56f4\u65e0\u6548\u3002", 
    "Invalid quota.": "\u914d\u989d\u65e0\u6548\u3002", 
    "Invitations": "Convites", 
    "Invite People": "Convidar Pessoas", 
    "Invite Time": "Tempo do Convite", 
    "Invite user": "\u9080\u8bf7\u7528\u6237", 
    "It is required.": "Obrigatorio", 
    "Just now": "Agora mesmo", 
    "Keep full history": "Manter hist\u00f3rico completo", 
    "Label": "\u6807\u7b7e", 
    "Label Current State": "\u6807\u8bb0\u5f53\u524d\u7248\u672c", 
    "Labels": "\u6807\u7b7e", 
    "Language": "Idioma", 
    "Language Setting": "Configura\u00e7\u00e3o de Idioma", 
    "Last Access": "\u00daltimo Acesso", 
    "Last Update": "\u00daltima atualiza\u00e7\u00e3o", 
    "Last modified by": "\u6700\u8fd1\u4fee\u6539", 
    "Leave": "\u9000\u51fa", 
    "Leave Group": "\u9000\u51fa\u7fa4\u7ec4", 
    "Leave Share": "Permitir compartilhamento", 
    "Less than or equal to": "\u5c0f\u4e8e\u6216\u7b49\u4e8e", 
    "Libraries": "Bibliotecas", 
    "Libraries shared as writable can be downloaded and synced by other group members. Read only libraries can only be downloaded, updates by others will not be uploaded.": "Bibliotecas compartilhadas no modo de escrita podem ser baixadas e sincronizadas por outros membros do grupo. Bibliotecas no modo de leitura pode ser apenas baixada, as altera\u00e7\u00f5es de outros n\u00e3o ser\u00e3o atualizadas.", 
    "Library": "Biblioteca", 
    "Library Name": "\u8d44\u6599\u5e93\u540d\u79f0", 
    "Library Type": "Tipo da Biblioteca", 
    "Library can not be shared to owner.": "\u8d44\u6599\u5e93\u65e0\u6cd5\u4e0e\u5176\u6240\u6709\u8005\u5171\u4eab\u3002", 
    "Limits": "Limites", 
    "Link": "Link", 
    "Link has been copied to clipboard": "\u94fe\u63a5\u5df2\u590d\u5236\u5230\u526a\u8d34\u677f", 
    "Link:": "\u94fe\u63a5\uff1a", 
    "Linked Devices": "Dispositivos Conectados", 
    "Links": "Links", 
    "List": "Lista", 
    "List your account in global address book, so that others can find you by typing your name.": "\u5728\u901a\u8baf\u5f55\u4e2d\u5217\u51fa\u4f60\u7684\u8d26\u6237, \u4ee5\u4fbf\u5176\u4ed6\u4eba\u53ef\u4ee5\u901a\u8fc7\u8f93\u5165\u4f60\u7684\u540d\u5b57\u627e\u5230\u4f60\u3002", 
    "Loading failed": "Falha no carregamento", 
    "Loading...": "Carregando...", 
    "Local draft": "\u672c\u5730\u8349\u7a3f", 
    "Location": "\u4f4d\u7f6e", 
    "Lock": "Trava", 
    "Log in": "Entrar", 
    "Log out": "Sair", 
    "Login again.": "\u518d\u6b21\u767b\u5f55\u3002", 
    "Logs": "Logs", 
    "Manage Members": "Gerenciar Membros", 
    "Manage group members": "\u7ba1\u7406\u7fa4\u7ec4\u6210\u5458", 
    "Mark as resolved": "\u6807\u8bb0\u4e3a\u5df2\u89e3\u51b3", 
    "Maximum number of files exceeded": "\u6587\u4ef6\u592a\u591a", 
    "Member": "Membro", 
    "Members": "Membros", 
    "Message (optional):": "Mensagem (opcional):", 
    "Modification Details": "Detalhes de modifica\u00e7\u00e3o", 
    "Modified files": "Arquivos modificados", 
    "Modifier": "Modificado", 
    "More": "Mais", 
    "More Operations": "Mais opera\u00e7\u00f5es", 
    "Move": "Mover", 
    "Move selected item(s) to:": "Mover itens selecionados para:", 
    "Move {num} items": "\u79fb\u52a8 {num} \u4e2a\u6761\u76ee", 
    "Move {placeholder} to": "\u79fb\u52a8 {placeholder} \u5230", 
    "Move {placeholder} to:": "Mover {placeholder} para:", 
    "Moved file": "\u79fb\u52a8\u6587\u4ef6", 
    "Moved folder": "\u79fb\u52a8\u6587\u4ef6\u5939", 
    "Moving %(name)s": "Movendo %(name)s", 
    "Moving file %(index)s of %(total)s": "Movendo %(index)s Arquivos de %(total)s", 
    "My Groups": "Meus grupos", 
    "My Libraries": "Minhas Bibliotecas", 
    "Name": "Nome", 
    "Name is required": "Nome requerido", 
    "Name is required.": "\u00c9 necess\u00e1rio fornecer nome.", 
    "Name should not include '/'.": "O Nome n\u00e3o pode incluir \"/\".", 
    "Network error": "\u7f51\u7edc\u9519\u8bef", 
    "New": "Novo", 
    "New Department": "\u65b0\u5efa\u90e8\u95e8", 
    "New Department Library": "\u65b0\u5efa\u90e8\u95e8\u8d44\u6599\u5e93", 
    "New Draft": "\u521b\u5efa\u8349\u7a3f", 
    "New Excel File": "Novo Arquivo Excel", 
    "New File": "Novo Arquivo", 
    "New Folder": "Nova Pasta", 
    "New Group": "Novo Grupo", 
    "New Library": "Nova biblioteca", 
    "New Markdown File": "Novo Arquivo Markdown", 
    "New Password": "Nova Senha", 
    "New Password Again": "Nova senha de novo", 
    "New PowerPoint File": "Novo Arquivo PowerPoint", 
    "New Sub-department": "\u65b0\u5efa\u5b50\u90e8\u95e8", 
    "New Tag": "\u65b0\u5efa\u6807\u7b7e", 
    "New Wiki": "\u65b0\u5efa\u7ef4\u57fa", 
    "New Word File": "Novo Arquivo Word", 
    "New directories": "Novos diret\u00f3rios", 
    "New file name": "\u65b0\u5efa\u6587\u4ef6\u540d\u5b57", 
    "New files": "Novos arquivos", 
    "New folder name": "\u65b0\u5efa\u6587\u4ef6\u5939\u540d\u5b57", 
    "New password has been sent to your email {mail}. Please check your mailbox. If you don\u2019t receive the password, please check if your email address is properly configured.": "\u65b0\u5bc6\u7801\u5df2\u53d1\u9001\u5230\u60a8\u7684\u7535\u5b50\u90ae\u4ef6{mail}\u3002 \u8bf7\u68c0\u67e5\u60a8\u7684\u90ae\u7bb1\u3002 \u5982\u679c\u60a8\u6ca1\u6709\u6536\u5230\u5bc6\u7801\uff0c\u8bf7\u68c0\u67e5\u60a8\u7684\u7535\u5b50\u90ae\u4ef6\u5730\u5740\u662f\u5426\u914d\u7f6e\u6b63\u786e\u3002", 
    "New password is too short": "A nova senha \u00e9 muito curta", 
    "New passwords don't match": "A nova senha e sua confirma\u00e7\u00e3o n\u00e3o s\u00e3o iguais", 
    "Next": "Pr\u00f3ximo", 
    "Next (Right arrow key)": "Proximo (Seta para a direita)", 
    "No comment yet.": "Nenhum coment\u00e1rio ainda.", 
    "No deleted libraries.": "\u6ca1\u6709\u5df2\u5220\u9664\u7684\u8d44\u6599\u5e93\u3002", 
    "No departments": "\u6ca1\u6709\u90e8\u95e8", 
    "No draft yet": "\u8fd8\u6ca1\u6709\u8349\u7a3f", 
    "No libraries": "Nenhuma biblioteca", 
    "No libraries have been shared directly with you. You can find more shared libraries at \"Shared with groups\".": "\u8fd8\u6ca1\u6709\u4efb\u4f55\u8d44\u6599\u5e93\u88ab\u76f4\u63a5\u5171\u4eab\u7ed9\u4f60\u3002\u4f60\u53ef\u4ee5\u5728 \"\u7fa4\u7ec4\u5171\u4eab\" \u4e2d\u770b\u5230\u66f4\u591a\u7684\u5171\u4eab\u8d44\u6599\u5e93\u3002", 
    "No libraries have been shared with you": "\u8fd8\u6ca1\u6709\u4efb\u4f55\u8d44\u6599\u5e93\u88ab\u5171\u4eab\u7ed9\u4f60", 
    "No library is shared to this group": "Nenhuma biblioteca est\u00e1 compartilhada com esse grupo", 
    "No matches": "Sem ocorr\u00eancias", 
    "No members": "Nenhum membro", 
    "No outline": "\u6ca1\u6709\u5927\u7eb2", 
    "No preview": "\u6ca1\u6709\u9884\u89c8", 
    "No public libraries": "\u6ca1\u6709\u516c\u5171\u8d44\u6599\u5e93", 
    "No result": "Sem resultado", 
    "No results matching.": "\u6ca1\u6709\u7ed3\u679c\u5339\u914d\u3002", 
    "No reviewer yet.": "\u8fd8\u6ca1\u6709\u8bc4\u5ba1\u4eba", 
    "No sub-departments": "\u6ca1\u6709\u5b50\u90e8\u95e8", 
    "None": "Nenhum", 
    "Notifications": "Notifica\u00e7\u00f5es", 
    "Number of changes:": "\u6587\u6863\u6539\u52a8\u6570\u91cf", 
    "Old Password": "Antiga senha", 
    "Online Read-Only": "\u4ec5\u4e91\u7aef\u9884\u89c8", 
    "Online Read-Only folder": "\u4ec5\u4e91\u7aef\u9884\u89c8\u6587\u4ef6\u5939", 
    "Online Read-Only library": "\u4ec5\u4e91\u7aef\u9884\u89c8\u8d44\u6599\u5e93", 
    "Online Read-Write": "\u4ec5\u4e91\u7aef\u8bfb\u5199", 
    "Online Read-Write folder": "\u4ec5\u4e91\u7aef\u8bfb\u5199\u6587\u4ef6\u5939", 
    "Online Read-Write library": "\u4ec5\u4e91\u7aef\u8bfb\u5199\u8d44\u6599\u5e93", 
    "Online view is not applicable to this file format": "\u6b64\u6587\u4ef6\u683c\u5f0f\u4e0d\u652f\u6301\u4e91\u7aef\u67e5\u770b", 
    "Only Show": "\u53ea\u663e\u793a", 
    "Only an extension there, please input a name.": "H\u00e1 apenas uma extens\u00e3o aqui, por favor, insira um nome.", 
    "Only keep a period of history:": "Mantenha apenas o per\u00edodo do hist\u00f3rico:", 
    "Open in New Tab": "Abrir Nova Aba", 
    "Open parent directory": "\u6253\u5f00\u7236\u76ee\u5f55", 
    "Open parent folder": "\u6253\u5f00\u7236\u76ee\u5f55", 
    "Open via Client": "Abrir atrav\u00e9s do cliente Desktop", 
    "Operation": "Opera\u00e7\u00e3o", 
    "Operations": "Opera\u00e7\u00f5es", 
    "Organization": "Organiza\u00e7\u00e3o", 
    "Organization Admin": "Administra\u00e7\u00e3o da Organiza\u00e7\u00e3o", 
    "Organizations": "Organiza\u00e7\u00f5es", 
    "Original file has been deleted.": "\u539f\u6587\u4ef6\u5df2\u88ab\u5220\u9664\u3002", 
    "Other Libraries": "Outras bibliotecas", 
    "Owned Libraries": "\u62e5\u6709\u7684\u8d44\u6599\u5e93", 
    "Owner": "Propriet\u00e1rio", 
    "Packaging...": "Empacotando...", 
    "Password": "Senha", 
    "Password again": "Informe a senha novamente", 
    "Password is required.": "Senha obrigatoria", 
    "Password is too short": "Senha muito curta", 
    "Password:": "Senha:", 
    "Passwords don't match": "Senhas n\u00e3o coincidem", 
    "Path": "\u6587\u4ef6\u8def\u5f84", 
    "Per 4 hours": "\u6bcf4\u5c0f\u65f6", 
    "Per day": "\u6bcf\u5929", 
    "Per hour": "\u6bcf\u5c0f\u65f6", 
    "Per week": "\u6bcf\u5468", 
    "Permission": "Permiss\u00e3o", 
    "Permission denied": "Permiss\u00e3o negada", 
    "Permission error": "Erro de Permiss\u00e3o", 
    "Platform": "Plataforma", 
    "Please check the network.": "Por favor ", 
    "Please choose a CSV file": "Por favor selecione um arquivo no formato CSV", 
    "Please choose an image file.": "\u8bf7\u9009\u62e9\u4e00\u4e2a\u56fe\u7247\u6587\u4ef6\u3002", 
    "Please click and choose a directory.": "Por favor clique e selecione um diret\u00f3rio.", 
    "Please enter 1 or more character": "Por favor  digite 1 ou mais caracteres", 
    "Please enter a new password": "Entre com a nova senha", 
    "Please enter a non-negative integer": "\u8bf7\u8f93\u5165\u4e00\u4e2a\u975e\u8d1f\u6574\u6570", 
    "Please enter days": "Por favor, informe os dias", 
    "Please enter days.": "Por favor insira os dias", 
    "Please enter password": "Por favor insira a senha", 
    "Please enter the new password again": "Por favor entre com a nova senha novamente", 
    "Please enter the old password": "Entre com a senha atual", 
    "Please enter the password again": "Por favor insira a senha novamente", 
    "Please enter valid days": "Por favor insira dias validos", 
    "Please input at least an email.": "Por favor, insira pelo menos um e-mail.", 
    "Please select a library to share.": "\u8bf7\u9009\u62e9\u4e00\u4e2a\u8d44\u6599\u5e93\u6765\u5171\u4eab\u3002", 
    "Please upload files less than {placeholder}M": "\u8bf7\u4e0a\u4f20\u5c0f\u4e8e {placeholder}M \u7684\u6587\u4ef6", 
    "Please upload no more than {maxFiles} files at a time.": "\u4e00\u6b21\u6700\u591a\u4e0a\u4f20  {maxFiles} \u4e2a\u6587\u4ef6\u3002", 
    "Preparing to upload...": "\u51c6\u5907\u4e0a\u4f20...", 
    "Preview and download": "\u9884\u89c8\u4e0e\u4e0b\u8f7d", 
    "Preview only": "\u4ec5\u67e5\u770b", 
    "Preview-Edit-on-Cloud": "\u4ec5\u4e91\u7aef\u8bfb\u5199", 
    "Preview-Edit-on-Cloud folder": "\u4ec5\u4e91\u7aef\u8bfb\u5199\u6587\u4ef6\u5939", 
    "Preview-Edit-on-Cloud library": "\u4ec5\u4e91\u7aef\u8bfb\u5199\u8d44\u6599\u5e93", 
    "Preview-on-Cloud": "\u4ec5\u4e91\u7aef\u9884\u89c8", 
    "Preview-on-Cloud folder": "\u4ec5\u4e91\u7aef\u9884\u89c8\u6587\u4ef6\u5939", 
    "Preview-on-Cloud library": "\u4ec5\u4e91\u7aef\u9884\u89c8\u8d44\u6599\u5e93", 
    "Previous": "Anterior", 
    "Previous (Left arrow key)": "Voltar (Seta para a esquerda)", 
    "Private": "\u79c1\u6709", 
    "Processing...": "Processando...", 
    "Profile": "Perfil", 
    "Profile Setting": "Configurar perfil", 
    "Public": "\u516c\u5f00", 
    "Public libraries are for publishing your contents in an organized way.": "\u516c\u5f00\u8d44\u6599\u5e93\u7528\u4e8e\u4ee5\u6709\u7ec4\u7ec7\u7684\u65b9\u5f0f\u53d1\u5e03\u60a8\u7684\u5185\u5bb9\u3002", 
    "Publish": "\u53d1\u5e03", 
    "Publish a Library": "\u53d1\u5e03\u8d44\u6599\u5e93", 
    "Publish draft": "\u53d1\u5e03\u8349\u7a3f", 
    "Published": "\u5df2\u53d1\u5e03", 
    "Published Libraries": "\u5df2\u53d1\u5e03\u7684\u8d44\u6599\u5e93", 
    "Quit Group": "Sair do Grupo", 
    "Quota": "\u914d\u989d", 
    "Quota is invalid.": "\u914d\u989d\u65e0\u6548\u3002", 
    "Read-Only": "Somente Leitura", 
    "Read-Only folder": "\u53ea\u8bfb\u6587\u4ef6\u5939", 
    "Read-Only library": "Biblioteca no modo somente leitura", 
    "Read-Only-Sync": "\u53ea\u8bfb\uff0c\u53ef\u540c\u6b65", 
    "Read-Write": "Leitura-Escrita", 
    "Read-Write folder": "\u53ef\u8bfb\u5199\u6587\u4ef6\u5939", 
    "Read-Write library": "Biblioteca no modo leitura-escrita", 
    "Read-Write-Sync": "\u8bfb\u5199\uff0c\u53ef\u540c\u6b65", 
    "Really want to delete this group?": "\u786e\u5b9a\u8981\u5220\u9664\u8fd9\u4e2a\u7fa4\u7ec4\uff1f", 
    "Really want to delete your account?": "Voc\u00ea realmente deseja excluir essa conta?", 
    "Really want to dismiss this group?": "Voc\u00ea realmente deseja ignorar este grupo?", 
    "Really want to leave this group?": "\u786e\u5b9a\u8981\u9000\u51fa\u8fd9\u4e2a\u7fa4\u7ec4\uff1f", 
    "Refresh": "Atualizar", 
    "Related Files": "\u76f8\u5173\u6587\u6863", 
    "Remaining": "\u5269\u4f59", 
    "Remove": "Remover", 
    "Removed all items from trash.": "\u5220\u9664\u4e86\u56de\u6536\u7ad9\u4e2d\u7684\u6240\u6709\u6761\u76ee.", 
    "Removed items older than {n} days from trash.": "\u5220\u9664\u4e86\u56de\u6536\u7ad9\u4e2d{n}\u5929\u524d\u7684\u6761\u76ee.", 
    "Rename": "Renomear", 
    "Rename File": "Renonear Arquivo", 
    "Rename Folder": "Renomear Pasta", 
    "Rename Group": "\u91cd\u547d\u540d\u7fa4\u7ec4", 
    "Rename group to": "\u91cd\u547d\u540d\u7fa4\u7ec4\u4e3a", 
    "Renamed file": "\u91cd\u547d\u540d\u6587\u4ef6", 
    "Renamed folder": "\u91cd\u547d\u540d\u6587\u4ef6\u5939", 
    "Renamed library": "\u91cd\u547d\u540d\u8d44\u6599\u5e93", 
    "Renamed or Moved files": "Arquivos renomeados ou movidos", 
    "Renaming {name} failed": "\u91cd\u547d\u540d{name}\u5931\u8d25", 
    "Replace": "Substituir", 
    "Replace file {filename}?": "Substituir arquivo {filename}?", 
    "Replacing it will overwrite its content.": "Substitu\u00ed-lo ir\u00e1 sobreescrever seu conte\u00fado.", 
    "Request a review": "\u8bf7\u6c42\u8bc4\u5ba1", 
    "Required at least three letters.": "\u81f3\u5c11\u9700\u8981\u4e09\u4e2a\u5b57\u6bcd\u3002", 
    "Reset": "\u91cd\u7f6e", 
    "Reset Password": "Resetar senha", 
    "Reset library password": "\u91cd\u7f6e\u8d44\u6599\u5e93\u5bc6\u7801", 
    "ResetPwd": "Restaurar senha", 
    "Resetting user's password, please wait for a moment.": "\u91cd\u7f6e\u7528\u6237\u5bc6\u7801\uff0c\u8bf7\u7a0d\u7b49\u3002", 
    "Restore": "Restaurar", 
    "Restore Library": "Restaurar Biblioteca", 
    "Restored file": "\u6062\u590d\u6587\u4ef6", 
    "Restored folder": "\u6062\u590d\u6587\u4ef6\u5939", 
    "Restored library": "\u6062\u590d\u8d44\u6599\u5e93", 
    "Retry": "\u91cd\u65b0\u4e0a\u4f20", 
    "Retry All": "\u91cd\u4f20\u6240\u6709\u5931\u8d25\u9879", 
    "Review": "\u8bc4\u5ba1", 
    "Reviewers": "\u8bc4\u5ba1\u4eba", 
    "Revoke Access": "\u64a4\u6d88\u8bbf\u95ee\u6743\u9650", 
    "Revoke Admin": "Administrador revogado", 
    "Role": "Papel", 
    "Save": "\u4fdd\u5b58", 
    "Save as ...": "\u53e6\u5b58\u4e3a...", 
    "Save to:": "\u6587\u4ef6\u53e6\u5b58\u4e3a:", 
    "Saving...": "Salvando...", 
    "Seafile": "Seafile", 
    "Search Files": "Pesquisar arquivos", 
    "Search files in this library": "Pesquisar arquivos nessa biblioteca", 
    "Search groups": "Procurar grupos", 
    "Search user or enter email and press Enter": "Digite o nome do usu\u00e1rio ou e-mail e pressione Enter", 
    "Search users": "\u641c\u7d22\u7528\u6237", 
    "Search users or enter emails and press Enter": "Digite o nome do usu\u00e1rio ou o e-mail e tecle Enter", 
    "Search...": "\u641c\u7d22...", 
    "Searching...": "Procurando...", 
    "See All Notifications": "Ver todas as Notifica\u00e7\u00f5e", 
    "Select File": "\u9009\u62e9\u6587\u4ef6", 
    "Select Image": "\u9009\u62e9\u56fe\u7247", 
    "Select Tags": "\u9009\u62e9\u6807\u7b7e", 
    "Select a color": "\u9009\u62e9\u4e00\u4e2a\u989c\u8272", 
    "Select a department": "\u9009\u62e9\u4e00\u4e2a\u90e8\u95e8", 
    "Select a group": "Selecionar um grupo", 
    "Select a user as admin...": "\u9009\u62e9\u4e00\u4e2a\u7528\u6237\u4f5c\u4e3a\u7ba1\u7406\u5458...", 
    "Select groups": "Selecionar grupos", 
    "Select groups...": "\u9009\u62e9\u7fa4\u7ec4...", 
    "Select libraries to share": "Selecione bibliotecas para compartilhar", 
    "Select related file for {placeholder}": "\u4e3a {placeholder} \u9009\u62e9\u76f8\u5173\u6587\u4ef6", 
    "Select users...": "\u9009\u62e9\u7528\u6237\u2026", 
    "Send": "Enviar", 
    "Send the invitation link to the others, and they will be able to join the organization via scanning the QR code.": "\u628a\u9080\u8bf7\u94fe\u63a5\u53d1\u7ed9\u5176\u4ed6\u4eba\uff0c\u5176\u4ed6\u4eba\u53ef\u4ee5\u626b\u7801\u52a0\u5165\u673a\u6784\u3002", 
    "Send to:": "Enviar para:", 
    "Sending new password...": "\u6b63\u5728\u53d1\u9001\u65b0\u5bc6\u7801...", 
    "Sending...": "Enviando\u2026", 
    "Server Version: ": "Vers\u00e3o do Servidor:", 
    "Set Password": "\u8bbe\u7f6e\u5bc6\u7801", 
    "Set Quota": "definir cota", 
    "Set permission": "\u8bbe\u7f6e\u6743\u9650", 
    "Set user contact email": "\u8bbe\u7f6e\u7528\u6237\u7684\u8054\u7cfb\u90ae\u7bb1", 
    "Set user name": "Defina o nome do usu\u00e1rio", 
    "Set user quota": "\u8bbe\u7f6e\u7528\u6237\u5bb9\u91cf", 
    "Set {placeholder}'s permission": "Setar {placeholder}'s  permiss\u00e3o", 
    "Setting library history is disabled by Admin.": "\u8d44\u6599\u5e93\u5386\u53f2\u8bbe\u7f6e\u5df2\u88ab\u7ba1\u7406\u5458\u7981\u7528\u3002", 
    "Settings": "Configura\u00e7\u00e3o", 
    "Share": "Compartilhar", 
    "Share Admin": "Compartilhar Administrador", 
    "Share From": "Compartilhamento de", 
    "Share Link": "Link compartilhado", 
    "Share Links": "Links compartilhados", 
    "Share To": "Compartilhar com", 
    "Share To:": "\u5171\u4eab\u7ed9\uff1a", 
    "Share existing libraries": "Compartilhar bibliotecas existentes", 
    "Share link is copied to the clipboard.": "\u5171\u4eab\u94fe\u63a5\u5df2\u590d\u5236\u5230\u526a\u8d34\u677f\u3002", 
    "Share to group": "Compartilhar com grupo", 
    "Share to user": "Compartilhar com usuario", 
    "Share {placeholder}": "Compartilhar {placeholder}", 
    "Shared By": "Compartilhado por", 
    "Shared Libraries": "\u5171\u4eab\u7684\u8d44\u6599\u5e93", 
    "Shared by:": "\u5171\u4eab\u6765\u6e90\uff1a", 
    "Shared by: ": "Compartilhado por:", 
    "Shared with all": "Compartilhado com todos", 
    "Shared with groups": "Compartilhado com grupos", 
    "Shared with me": "Compartilhado comigo", 
    "Show": "Mostrar", 
    "Show Codes": "Exibir C\u00f3digos", 
    "Show resolved comments": "\u663e\u793a\u5df2\u89e3\u51b3\u7684\u8bc4\u8bba", 
    "Side Nav Menu": "Menu de Navega\u00e7\u00e3o Lateral", 
    "Size": "Tamanho", 
    "Social Login": "\u793e\u4ea4\u8d26\u53f7\u767b\u5f55", 
    "Space Used": "Espa\u00e7o utilizado", 
    "Space Used / Quota": "Espa\u00e7o utilizado / cota", 
    "Star": "Marcar", 
    "Start": "Iniciar", 
    "Start date should be earlier than end date.": "\u521d\u59cb\u65e5\u671f\u5e94\u8be5\u65e9\u4e8e\u7ed3\u675f\u65e5\u671f\u3002", 
    "Start review": "\u5f00\u59cb\u8bc4\u5ba1", 
    "Statistic": "\u7edf\u8ba1", 
    "Status": "Estado", 
    "Status: enabled": "\u72b6\u6001\uff1a\u5df2\u542f\u7528", 
    "Storage Backend": "\u5b58\u50a8\u540e\u7aef", 
    "Sub-departments": "\u5b50\u90e8\u95e8", 
    "Submit": "Enviar", 
    "Success": "Sucesso", 
    "Successfully added label(s) for library {placeholder}": "\u7ed9\u8d44\u6599\u5e93 {placeholder} \u6dfb\u52a0\u6807\u7b7e\u6210\u529f", 
    "Successfully changed library password.": "Senha da biblioteca alterada com sucesso.", 
    "Successfully clean all errors.": "Todos os erros foram eliminados com sucesso.", 
    "Successfully copied %(name)s": "%(name)s copiado com sucesso", 
    "Successfully copied %(name)s and %(amount)s other items.": "%(name)s and %(amount)s outros items.copiados com sucesso", 
    "Successfully copied %(name)s and 1 other item.": "%(name)s  copiado com sucesso e  1 outro items", 
    "Successfully copied %(name)s.": " %(name)s. copiados com sucesso", 
    "Successfully copied files to another library.": "\u6210\u529f\u590d\u5236\u6587\u4ef6\u5230\u53e6\u4e00\u4e2a\u8d44\u6599\u5e93\u3002", 
    "Successfully deleted %(name)s": "%(name)s exclu\u00eddo com sucesso", 
    "Successfully deleted %(name)s and %(amount)s other items.": "Exclu\u00eddo com \u00eaxito% (name)s e %(amount) s outros itens.", 
    "Successfully deleted %(name)s and 1 other item.": "Sucesso ao excluir %(name)s e 1 outro item.", 
    "Successfully deleted %(name)s.": "%(name)s exclu\u00eddos com sucesso.", 
    "Successfully deleted %s": "%s exclu\u00eddo com sucesso", 
    "Successfully deleted 1 item": "Sucesso ao excluir 1 item.", 
    "Successfully deleted 1 item.": "Item exclu\u00eddo com sucesso.", 
    "Successfully deleted draft %(draft)s.": "\u6210\u529f\u5220\u9664\u8349\u7a3f%(draft)s\u3002", 
    "Successfully deleted library {placeholder}": "\u6210\u529f\u5220\u9664\u8d44\u6599\u5e93{placeholder}", 
    "Successfully deleted member {placeholder}": "Sucesso ao exclu\u00eddo membro {placeholder}", 
    "Successfully deleted {name}": " {name}  \u5220\u9664\u6210\u529f", 
    "Successfully deleted {name} and other {n} items.": "\u6210\u529f\u5220\u9664 {name} \u548c\u5176\u4ed6 {n} \u9879\u5185\u5bb9\u3002", 
    "Successfully deleted {name}.": " {name}  \u5220\u9664\u6210\u529f\u3002", 
    "Successfully deleted.": "Exclu\u00eddo com sucesso.", 
    "Successfully edited labels.": "\u7f16\u8f91\u6807\u7b7e\u6210\u529f\u3002", 
    "Successfully imported.": "Importado com sucesso.", 
    "Successfully invited %(email) and %(num) other people.": "\u6210\u529f\u9080\u8bf7\u4e86 %(email) \u548c\u5176\u4ed6 %(num) \u4e2a\u4eba\u3002", 
    "Successfully invited %(email).": "\u6210\u529f\u9080\u8bf7\u4e86  %(email)\u3002", 
    "Successfully modified permission": "Permiss\u00f5es modificadas com sucesso", 
    "Successfully modified permission.": "\u6210\u529f\u66f4\u6539\u6743\u9650\u3002", 
    "Successfully moved %(name)s": "%(name)s movido com sucesso", 
    "Successfully moved %(name)s and %(amount)s other items.": "%(name)s e %(amount)s outros items.copiados com sucesso", 
    "Successfully moved %(name)s and 1 other item.": "%(name)s e  1 outro item.copiados com sucesso", 
    "Successfully moved %(name)s.": "Movidos com sucesso %(name)s.", 
    "Successfully moved files to another library.": "\u6210\u529f\u79fb\u52a8\u6587\u4ef6\u5230\u53e6\u4e00\u4e2a\u8d44\u6599\u5e93\u3002", 
    "Successfully moved {name}.": "\u6210\u529f\u79fb\u52a8 {name}\u3002", 
    "Successfully published draft %(draft)s.": "\u6210\u529f\u53d1\u5e03\u8349\u7a3f%(draft)s\u3002", 
    "Successfully reset password to %(passwd)s for user %(user)s.": "Reiniciada com sucesso a senha para %(passwd)s para o usu\u00e1rio %(user)s.", 
    "Successfully restored 1 item.": "Sucesso ao restaurar 1 item. ", 
    "Successfully restored library {placeholder}": "\u6210\u529f\u8fd8\u539f\u8d44\u6599\u5e93 {placeholder}", 
    "Successfully restored the library.": "\u8d44\u6599\u5e93\u8fd8\u539f\u6210\u529f\u3002", 
    "Successfully restored.": "\u6062\u590d\u6210\u529f\u3002", 
    "Successfully revoke the admin permission of %s": "A permiss\u00e3o de administrador do usu\u00e1rio %s foi revogada com sucesso", 
    "Successfully revoked access of user {placeholder}.": "\u5df2\u6210\u529f\u64a4\u6d88\u7528\u6237 {placeholder} \u7684\u8bbf\u95ee\u6743\u9650\u3002", 
    "Successfully saved": "\u4fdd\u5b58\u6210\u529f", 
    "Successfully sent to {placeholder}": "Enviado com sucesso para {placeholder}", 
    "Successfully set %s as admin.": "%s foi definido como administrador com sucesso", 
    "Successfully set library history.": "Hist\u00f3rico da biblioteca atualizado com sucesso.", 
    "Successfully starred {library_name_placeholder}.": "\u6210\u529f\u6dfb\u52a0\u661f\u6807 {library_name_placeholder}", 
    "Successfully transferred the group.": "Grupo transferido com sucesso.", 
    "Successfully transferred the group. You are now a normal member of the group.": "O grupo foi transferido com sucesso. Voc\u00ea agora \u00e9 um membro regular do grupo.", 
    "Successfully transferred the library.": "Biblioteca transferida com sucesso.", 
    "Successfully unlink %(name)s.": "O dispositivo %(name)s foi desconectado com sucesso.", 
    "Successfully unlinked %(name)s.": "\u6210\u529f\u65ad\u5f00\u8fde\u63a5 %(name)s \u3002", 
    "Successfully unshared 1 item.": "O compartilhamento foi removido com sucesso.", 
    "Successfully unshared library {placeholder}": "Sucesso ao descompartilhar biblioteca {placeholder}", 
    "Successfully unshared {name}": "\u6210\u529f\u53d6\u6d88\u5171\u4eab {name}", 
    "Successfully unstared {placeholder}": "{placeholder} desmarcado com sucesso", 
    "Successfully unstarred {library_name_placeholder}.": "\u6210\u529f\u53d6\u6d88\u661f\u6807 {library_name_placeholder}", 
    "Suggestion": "\u5efa\u8bae", 
    "Switch to plain text editor": "\u5207\u6362\u81f3\u666e\u901a\u6587\u672c\u7f16\u8f91\u5668", 
    "Switch to rich text editor": "\u5207\u6362\u81f3\u5bcc\u6587\u672c\u7f16\u8f91\u5668", 
    "System Admin": "Administrador do sistema", 
    "Tag \"{name}\" already exists.": "\u6807\u7b7e \"{name}\" \u5df2\u7ecf\u5b58\u5728\u3002", 
    "Tag should not include ','.": "\u6807\u7b7e\u4e0d\u5e94\u8be5\u5305\u542b ','\u3002", 
    "Tagged Files": "\u6253\u6807\u7b7e\u7684\u6587\u4ef6", 
    "Tags": "\u6807\u7b7e", 
    "Terms and Conditions": "Termos e Condi\u00e7\u00f5es", 
    "Text": "Texto", 
    "Text files": "Arquivo texto", 
    "The draft already exists.": "\u6587\u4ef6\u8349\u7a3f\u5df2\u5b58\u5728\u3002", 
    "The file is too large. Allowed maximum size is 1MB.": " \u6587\u4ef6\u592a\u5927\u3002\u4e0d\u5f97\u8d85\u8fc7 1MB\u3002", 
    "The image could not be loaded.": "\u56fe\u7247\u65e0\u6cd5\u88ab\u52a0\u8f7d\u3002", 
    "The link is copied to the clipboard.": "\u5916\u94fe\u5df2\u590d\u5236\u5230\u526a\u8d34\u677f\u3002", 
    "The list of added, deleted and modified files will be sent to your mailbox.": "\u6dfb\u52a0\uff0c\u5220\u9664\u548c\u4fee\u6539\u7684\u6587\u4ef6\u5217\u8868\u5c06\u53d1\u9001\u5230\u60a8\u7684\u90ae\u7bb1\u3002", 
    "The name \"{name}\" is already taken. Please choose a different name.": "\u540d\u79f0\u201c{name}\u201d\u5df2\u88ab\u5360\u7528\uff0c\u8bf7\u9009\u53d6\u4e00\u4e2a\u4e0d\u540c\u7684\u540d\u5b57\u3002", 
    "The password will be kept in the server for only 1 hour.": "A senha ser\u00e1 mantida no servidor por apenas 1 hora.", 
    "This file has been updated.": "\u8fd9\u4e2a\u6587\u4ef6\u5df2\u88ab\u4fee\u6539\u3002", 
    "This file is in draft stage.": "\u8be5\u6587\u4ef6\u5904\u4e8e\u8349\u7a3f\u9636\u6bb5\u3002", 
    "This folder has no content at this time.": "\u8be5\u6587\u4ef6\u5939\u6682\u65e0\u4efb\u4f55\u5185\u5bb9\u3002", 
    "This is a draft": "\u8fd9\u662f\u8349\u7a3f\u6587\u4ef6", 
    "This is a special group representing a department.": "\u8be5\u7fa4\u7ec4\u4ee3\u8868\u4e00\u4e2a\u90e8\u95e8\u3002", 
    "This library has been set to read-only by admin and cannot be updated.": "\u6b64\u5e93\u5df2\u7531\u7ba1\u7406\u5458\u8bbe\u7f6e\u4e3a\u53ea\u8bfb\uff0c\u65e0\u6cd5\u66f4\u65b0\u3002", 
    "This library is password protected": "Esta biblioteca esta protegida com senha", 
    "This operation will not be reverted. Please think twice!": "Essa opera\u00e7\u00e3o n\u00e3o ser\u00e1 revertida. Favor verifique duas vezes!", 
    "Time": "Hora", 
    "Tip: -2 means no limit.": "\u63d0\u793a\uff1a-2 \u8868\u793a\u6ca1\u6709\u9650\u5236\u3002", 
    "Tip: 0 means default limit": "Dica: 0 significa limite padr\u00e3o", 
    "Tip: a snapshot will be generated after modification, which records the library state after the modification.": "Dica: um snapshot ser\u00e1 gerado ap\u00f3s cada modifica\u00e7\u00e3o, registrando o estado da biblioteca ap\u00f3s a modifica\u00e7\u00e3o.", 
    "Tip: libraries deleted 30 days ago will be cleaned automatically.": "Dica: bibliotecas exclu\u00edda h\u00e1 mais de 30 dias ser\u00e3o automaticamente removidas.", 
    "Tools": "Ferramentas", 
    "Total Users": "Total de usu\u00e1rios", 
    "Transfer": "Transferir", 
    "Transfer Group": "Transferir Grupo", 
    "Transfer Group {group_name} To": "Transferir Grupo {group_name} para", 
    "Transfer Library": "Tranferir Biblioteca", 
    "Transfer Library {library_name}": "\u8f6c\u8ba9\u8d44\u6599\u5e93 {library_name}", 
    "Transfer Library {library_name} To": "Transferir Biblioteca {library_name} para", 
    "Transfer group to": "\u8f6c\u6362\u7fa4\u7ec4\u7ed9", 
    "Transfer to department": "\u8f6c\u8ba9\u7ed9\u90e8\u95e8", 
    "Transfer to user": "\u8f6c\u8ba9\u7ed9\u7528\u6237", 
    "Transferred group {group_name} from {user_from} to {user_to}": "Grupo transferido {group_name} de {user_from} para {user_to}", 
    "Transferred library {library_name} from {user_from} to {user_to}": "Biblioteca transferida {library_name} de {user_from} para {user_to}", 
    "Trash": "Lixeira", 
    "Two-Factor Authentication": "Autentica\u00e7\u00e3o de 2 fatores", 
    "Two-factor authentication is not enabled for your account. Enable two-factor authentication for enhanced account security.": "Autentica\u00e7\u00e3o de 2 fatores n\u00e3o est\u00e1 habilitada para sua conta. Habilite esse tipo de autentica\u00e7\u00e3o para aumentar a seguran\u00e7a da sua conta.", 
    "Type": "Tipo", 
    "Unknown": "Desconhecido", 
    "Unlink": "Desvincular", 
    "Unlink device": "Desconectar dispositivo", 
    "Unlock": "Destravar", 
    "Unpublish": "\u53d6\u6d88\u53d1\u5e03", 
    "Unpublish Library": "\u53d6\u6d88\u53d1\u5e03\u8d44\u6599\u5e93", 
    "Unresolved comments:": "\u672a\u5904\u7406\u8bc4\u8bba\uff1a", 
    "Unshare": "Descompartilhar", 
    "Unshare Library": "Descompartilhar Biblioteca", 
    "Unstar": "Desmarcar", 
    "Update": "Atualizar", 
    "Updated draft": "\u66f4\u65b0\u8349\u7a3f", 
    "Updated file": "\u66f4\u65b0\u6587\u4ef6", 
    "Upload": "Enviar arquivo", 
    "Upload Files": "Enviar arquivos", 
    "Upload Folder": "Pasta de Upload", 
    "Upload Link": "Enviar Link", 
    "Upload Link:": "\u4e0a\u4f20\u8fde\u63a5\uff1a", 
    "Upload Links": "Enviar Links", 
    "Upload link is copied to the clipboard.": "\u4e0a\u4f20\u8fde\u63a5\u5df2\u590d\u5236\u5230\u526a\u5207\u677f\u3002", 
    "Uploaded": "\u5df2\u4e0a\u4f20", 
    "Uploaded bytes exceed file size": "Bytes enviados excedem o tamanho do arquivo", 
    "Use draft": "\u4f7f\u7528\u8349\u7a3f", 
    "Used:": "Utilizado:", 
    "User": "Usu\u00e1rio", 
    "User Permission": "Permiss\u00e3o de Usuario", 
    "User can only view files online via browser. Files can't be downloaded.": "\u7528\u6237\u53ea\u80fd\u901a\u8fc7\u6d4f\u89c8\u5668\u5728\u7ebf\u67e5\u770b\u6587\u4ef6\u3002\u6587\u4ef6\u4e0d\u80fd\u88ab\u4e0b\u8f7d\u3002", 
    "User can read, download and sync files.": "\u7528\u6237\u53ef\u4ee5\u67e5\u770b\u3001\u4e0b\u8f7d\u548c\u540c\u6b65\u6587\u4ef6\u3002", 
    "User can read, write, upload, download and sync files.": "\u7528\u6237\u53ef\u4ee5\u67e5\u770b\u3001\u4fee\u6539\u3001\u4e0a\u4f20\u3001\u4e0b\u8f7d\u548c\u540c\u6b65\u6587\u4ef6\u3002", 
    "User can view and edit file online via browser. Files can't be downloaded.": "\u7528\u6237\u53ef\u4ee5\u901a\u8fc7\u6d4f\u89c8\u5668\u5728\u7ebf\u67e5\u770b\u548c\u7f16\u8f91\u6587\u4ef6\u3002 \u6587\u4ef6\u4e0d\u80fd\u88ab\u4e0b\u8f7d\u3002", 
    "User not found": "\u672a\u627e\u5230\u7528\u6237", 
    "User {user_link} has added you to {group_link}": "\u7528\u6237 {user_link} \u5c06\u4f60\u52a0\u5165\u5230\u7fa4\u7ec4 {group_link}\u3002", 
    "Username:": "Nome de usu\u00e1rio:", 
    "Users": "Usu\u00e1rios", 
    "Version": "Vers\u00e3o", 
    "Video": "V\u00eddeos", 
    "View": "Ver", 
    "View Draft": "\u67e5\u770b\u8349\u7a3f", 
    "View Link": "\u67e5\u770b\u94fe\u63a5", 
    "View Snapshot": "Ver inst\u00e2ntanea", 
    "View diff": "\u67e5\u770b\u5bf9\u6bd4", 
    "View-on-Cloud": "\u4ec5\u4e91\u7aef\u53ea\u8bfb", 
    "Virus Scan": "Verifica\u00e7\u00e3o de V\u00edrus", 
    "Visits": "Visitas", 
    "WebDav Password": "webDav\u5bc6\u7801", 
    "Wrong password": "Senha incorreta", 
    "You are logged out.": "\u4f60\u5df2\u7ecf\u767b\u51fa\u3002", 
    "You are not in any groups": "Voc\u00ea n\u00e3o est\u00e1 em nenhum grupo", 
    "You can create a library to organize your files. For example, you can create one for each of your projects. Each library can be synchronized and shared separately.": "Voc\u00ea pode criar uma biblioteca para organizar seus arqquivos. Por exemplo, voc\u00ea pode criar uma para cada projeto. Cada biblioteca pode ser soncronizada e compartilhada separadamente. ", 
    "You can create a public library by clicking the \"New Library\" button, others can view and download this library.": "\u4f60\u53ef\u4ee5\u901a\u8fc7\u70b9\u51fb\u201c\u65b0\u5efa\u8d44\u6599\u5e93\u201d\u6309\u94ae\u6765\u521b\u5efa\u4e00\u4e2a\u516c\u5171\u8d44\u6599\u5e93\uff0c\u5176\u4ed6\u4eba\u53ef\u4ee5\u67e5\u770b\u548c\u4e0b\u8f7d\u8be5\u8d44\u6599\u5e93\u3002", 
    "You can create files quickly": "\u4f60\u53ef\u4ee5\u5feb\u901f\u521b\u5efa\u6587\u4ef6", 
    "You can create libraries by clicking the \"New Library\" button above.": "\u4f60\u53ef\u4ee5\u70b9\u51fb\u4e0a\u9762\u7684\u201c\u65b0\u5efa\u8d44\u6599\u5e93\u201d\u6309\u626d\u521b\u5efa\u8d44\u6599\u5e93\u3002", 
    "You can generate a share link for a folder or a file. Anyone who receives this link can view the folder or the file online.": "\u60a8\u53ef\u4ee5\u4e3a\u6587\u4ef6\u5939\u6216\u6587\u4ef6\u751f\u6210\u5171\u4eab\u94fe\u63a5\u3002\u4e0d\u7ba1\u8c01\u63a5\u6536\u8fd9\u4e2a\u94fe\u63a5\uff0c\u90fd\u53ef\u4ee5\u5728\u7ebf\u67e5\u770b\u5bf9\u5e94\u7684\u6587\u4ef6\u5939\u6216\u6587\u4ef6\u3002", 
    "You can generate an upload link from any folder. Anyone who receives this link can upload files to this folder.": "\u4f60\u53ef\u4ee5\u7ed9\u4efb\u610f\u6587\u4ef6\u5939\u751f\u6210\u4e0a\u4f20\u5916\u94fe\u3002\u522b\u4eba\u901a\u8fc7\u8fd9\u4e2a\u5916\u94fe\u5c31\u53ef\u4ee5\u4e0a\u4f20\u6587\u4ef6\u5230\u8be5\u6587\u4ef6\u5939\u3002", 
    "You can only select 1 item": "Voc\u00ea pode selecionar apenas 1 item", 
    "You can share a single folder with a registered user if you don't want to share a whole library.": "Voc\u00ea pode compartilhar uma pasta \u00fanica com usu\u00e1rio se voc\u00ea n\u00e3o quiser compartilhar uma biblioteca inteira.", 
    "You can share libraries by clicking the \"New Library\" button above or the \"Share\" icon on your libraries list.": "Voc\u00ea pode compartilhar bibliotecas clicando no bot\u00e3o \"Nova biblioteca\" ou no \u00edcone \"Compartilhar\" na sua lista de bibliotecas.", 
    "You can share libraries with your friends and colleagues by clicking the share icon of your own libraries in your home page or creating a new library in groups you are in.": "\u4f60\u53ef\u4ee5\u901a\u8fc7\u5728\u4f60\u7684\u4e2a\u4eba\u9875\u9762\u70b9\u51fb\u4f60\u81ea\u5df1\u7684\u8d44\u6599\u5e93\u7684\u5171\u4eab\u56fe\u6807, \u6216\u662f\u5728\u4f60\u6240\u5728\u7684\u5c0f\u7ec4\u91cc\u521b\u5efa\u4e00\u4e2a\u65b0\u7684\u8d44\u6599\u5e93\u6765\u5c06\u4f60\u7684\u8d44\u6599\u5e93\u5171\u4eab\u7ed9\u4f60\u7684\u670b\u53cb\u548c\u540c\u4e8b\u3002", 
    "You can use this field at login.": "Voc\u00ea pode usar este campo no login.", 
    "You cannot select any more choices": "Voc\u00ea n\u00e3o pode selecionar mais op\u00e7\u00f5es", 
    "You do not have any public library": "\u4f60\u8fd8\u672a\u521b\u5efa\u4efb\u4f55\u516c\u5f00\u8d44\u6599\u5e93", 
    "You do not have connected devices": "Voc\u00ea n\u00e3o tem dispositivos conectados", 
    "You have an unsaved draft. Do you like to use it?": "\u6709\u672a\u4fdd\u5b58\u7684\u8349\u7a3f\uff0c\u4f7f\u7528\u8349\u7a3f\u5417\uff1f", 
    "You have logged out.": "Voc\u00ea encerrou a sess\u00e3o.", 
    "You have not created any libraries": "Voc\u00ea n\u00e3o criou bibliotecas", 
    "You have not invited any people.": "Voc\u00ea n\u00e3o convidou ningu\u00e9m.", 
    "You have not shared any libraries": "\u4f60\u8fd8\u6ca1\u6709\u5171\u4eab\u4efb\u4f55\u8d44\u6599\u5e93", 
    "You have only one backup code remaining.": "\u4f60\u53ea\u6709\u4e00\u4e2a\u5907\u7528\u9a8c\u8bc1\u7801\u3002", 
    "You have {num} backup codes remaining.": "\u4f60\u8fd8\u5269\u4f59 {num} \u4e2a\u5907\u7528\u9a8c\u8bc1\u7801\u3002", 
    "Your clients (Desktop/Android/iOS) will be listed here.": "Seus clientes (Desktop/Android/iOS) ser\u00e3o listados aqui.", 
    "Your notifications will be sent to this email.": "Suas notifica\u00e7\u00f5es ser\u00e3o enviadas para este e-mail.", 
    "Zoom in": "\u7f29\u5c0f", 
    "Zoom out": "\u653e\u5927", 
    "all": "todos", 
    "all members": "Todos os membros", 
    "canceled": "Cancelado", 
    "days": "dias", 
    "deleted": "\u5df2\u5220\u9664", 
    "details": "\u8be6\u60c5", 
    "draft": "\u8349\u7a3f", 
    "email is required": "\u8bf7\u8f93\u5165\u90ae\u7bb1", 
    "file": "arquivo", 
    "files": "\u6587\u4ef6", 
    "icon": "\u00edcone", 
    "locked": "Travado", 
    "locked by {name}": "\u88ab {name} \u9501\u5b9a", 
    "locked by {placeholder}": "Bloqueado por {placeholder}", 
    "name": "nome", 
    "progress": "\u8fdb\u5ea6", 
    "related files": "\u76f8\u5173\u6587\u4ef6", 
    "result": "\u7ed3\u679c", 
    "results": "\u7ed3\u679c", 
    "show more": "\u663e\u793a\u66f4\u591a", 
    "size": "\u5927\u5c0f", 
    "starred": "marcado", 
    "state": "\u72b6\u6001", 
    "successfully added user %s.": "\u6210\u529f\u6dfb\u52a0\u7528\u6237 %s\u3002", 
    "to": "para", 
    "unstarred": "desmarcar", 
    "uploaded": "Carregado", 
    "you can also press \u2190 ": "voc\u00ea pode tamb\u00e9m pressionar \u2190 ", 
    "you can also press \u2192": "\u4f60\u4e5f\u53ef\u4ee5\u6309\u2192", 
    "{draft_link} has a new comment from user {author}.": "\u7528\u6237 {author} \u7ed9 {draft_link} \u6dfb\u52a0\u4e86\u4e00\u4e2a\u65b0\u7684\u8bc4\u8bba\u3002", 
    "{file} and {n} other files": "{file} \u53ca {n} \u4e2a\u5176\u4ed6\u6587\u4ef6", 
    "{from_user} has sent you a request for {draft_link}.": "{from_user} \u8bf7\u6c42\u4f60\u8bc4\u5ba1 {draft_link} \u3002", 
    "{placeholder} Folder Permission": "{placeholder} Permiss\u00e3o da Pasta", 
    "{placeholder} History Setting": "{placeholder} Configura\u00e7\u00e3o do Hist\u00f3rico", 
    "{placeholder} Members": "{placeholder} Membros", 
    "{placeholder} Modification History": "{placeholder}\u4fee\u6539\u5386\u53f2\u8bb0\u5f55", 
    "{placeholder} Share Links": "{placeholder}  Compartilhar Links", 
    "{placeholder} Snapshot": "{placeholder} \u955c\u50cf", 
    "{placeholder} Trash": "{placeholder} \u56de\u6536\u7ad9", 
    "{share_from} has shared a folder named {repo_link} to group {group_link}.": "{share_from} \u7ed9\u7fa4\u7ec4 {group_link} \u5171\u4eab\u4e86\u4e00\u4e2a\u6587\u4ef6\u5939 {group_link}\u3002", 
    "{share_from} has shared a folder named {repo_link} to you.": "{share_from} \u7ed9\u4f60\u5171\u4eab\u4e86\u4e00\u4e2a\u540d\u4e3a {repo_link} \u7684\u6587\u4ef6\u5939\u3002", 
    "{share_from} has shared a library named {repo_link} to group {group_link}.": "{share_from} \u7ed9\u7fa4\u7ec4 {group_link} \u5171\u4eab\u4e86\u4e00\u4e2a\u8d44\u6599\u5e93 {repo_link}\u3002", 
    "{share_from} has shared a library named {repo_link} to you.": "{share_from} \u5171\u4eab\u4e86\u8d44\u6599\u5e93 {repo_link} \u7ed9\u4f60\u3002", 
    "{user} has transfered a library named {repo_link} to you.": "{user} \u8f6c\u8ba9\u4e86\u4e00\u4e2a\u8d44\u6599\u5e93 {repo_link} \u7ed9\u4f60\u3002"
  };
  for (var key in newcatalog) {
    django.catalog[key] = newcatalog[key];
  }
  

  if (!django.jsi18n_initialized) {
    django.gettext = function(msgid) {
      var value = django.catalog[msgid];
      if (typeof(value) == 'undefined') {
        return msgid;
      } else {
        return (typeof(value) == 'string') ? value : value[0];
      }
    };

    django.ngettext = function(singular, plural, count) {
      var value = django.catalog[singular];
      if (typeof(value) == 'undefined') {
        return (count == 1) ? singular : plural;
      } else {
        return value[django.pluralidx(count)];
      }
    };

    django.gettext_noop = function(msgid) { return msgid; };

    django.pgettext = function(context, msgid) {
      var value = django.gettext(context + '\x04' + msgid);
      if (value.indexOf('\x04') != -1) {
        value = msgid;
      }
      return value;
    };

    django.npgettext = function(context, singular, plural, count) {
      var value = django.ngettext(context + '\x04' + singular, context + '\x04' + plural, count);
      if (value.indexOf('\x04') != -1) {
        value = django.ngettext(singular, plural, count);
      }
      return value;
    };

    django.interpolate = function(fmt, obj, named) {
      if (named) {
        return fmt.replace(/%\(\w+\)s/g, function(match){return String(obj[match.slice(2,-2)])});
      } else {
        return fmt.replace(/%s/g, function(match){return String(obj.shift())});
      }
    };


    /* formatting library */

    django.formats = {
    "DATETIME_FORMAT": "j \\d\\e F \\d\\e Y \u00e0\\s H:i", 
    "DATETIME_INPUT_FORMATS": [
      "%d/%m/%Y %H:%M:%S", 
      "%d/%m/%Y %H:%M:%S.%f", 
      "%d/%m/%Y %H:%M", 
      "%d/%m/%Y", 
      "%d/%m/%y %H:%M:%S", 
      "%d/%m/%y %H:%M:%S.%f", 
      "%d/%m/%y %H:%M", 
      "%d/%m/%y", 
      "%Y-%m-%d %H:%M:%S", 
      "%Y-%m-%d %H:%M:%S.%f", 
      "%Y-%m-%d %H:%M", 
      "%Y-%m-%d"
    ], 
    "DATE_FORMAT": "j \\d\\e F \\d\\e Y", 
    "DATE_INPUT_FORMATS": [
      "%d/%m/%Y", 
      "%d/%m/%y", 
      "%Y-%m-%d"
    ], 
    "DECIMAL_SEPARATOR": ",", 
    "FIRST_DAY_OF_WEEK": "0", 
    "MONTH_DAY_FORMAT": "j \\d\\e F", 
    "NUMBER_GROUPING": "3", 
    "SHORT_DATETIME_FORMAT": "d/m/Y H:i", 
    "SHORT_DATE_FORMAT": "d/m/Y", 
    "THOUSAND_SEPARATOR": ".", 
    "TIME_FORMAT": "H:i", 
    "TIME_INPUT_FORMATS": [
      "%H:%M:%S", 
      "%H:%M:%S.%f", 
      "%H:%M"
    ], 
    "YEAR_MONTH_FORMAT": "F \\d\\e Y"
  };

    django.get_format = function(format_type) {
      var value = django.formats[format_type];
      if (typeof(value) == 'undefined') {
        return format_type;
      } else {
        return value;
      }
    };

    /* add to global namespace */
    globals.pluralidx = django.pluralidx;
    globals.gettext = django.gettext;
    globals.ngettext = django.ngettext;
    globals.gettext_noop = django.gettext_noop;
    globals.pgettext = django.pgettext;
    globals.npgettext = django.npgettext;
    globals.interpolate = django.interpolate;
    globals.get_format = django.get_format;

    django.jsi18n_initialized = true;
  }

}(this));

